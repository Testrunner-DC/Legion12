...
public sealed class ControlPlanePhaseThreeStorageTests
{
    ...
    public void LegacyJsonMigratesWithBackupIntegrityAndRecoveryRehearsal()
    {
        ...
        try
        {
            ...
            using (var connection = Open(store.TransactionalStoragePath))
            {
                ...
                Assert.Equal("6", Scalar(connection, "SELECT value FROM storage_meta WHERE key='schema_version';"));
            }
    ...
    public void SchemaMarkersUpgradeWithoutLosingOperationsConfiguration()
    {
        ...
        try
        {
            ...
            Assert.Equal("6", Scalar(upgraded, "SELECT value FROM storage_meta WHERE key='schema_version';"));
            Assert.Equal("6", Scalar(upgraded, "SELECT schema_version FROM platform_state WHERE singleton_id=1;"));
        }
        finally { Directory.Delete(root, true); }
    }

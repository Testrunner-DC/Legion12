...
const contracts = [
  ...
  [l12Net.includes("send({ type: 'sandboxAction', actingPlayerIndex, command })") && board.includes('controlledPlayerIndex') && board.includes('watch(controlledPlayerIndex') && board.includes('v-if="l12State.gmEnabled" :cards="enemy.hand"') && prompt.includes('sandboxAction(actingPlayerIndex, command)'), '沙盒控制者必须可查看双方手牌、清理切换残留并代行双方全部规则内选择'],

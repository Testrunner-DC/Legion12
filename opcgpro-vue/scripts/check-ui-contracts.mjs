...
const contracts = [
  ...
  [board.includes('<Teleport to="body">') && board.includes('public-card-reveal-animation') && board.includes('z-index:2147483000') && board.includes("event.type === 'effect-trigger'") && board.includes("event.type === 'reveal'") && board.includes("event.type === 'effect-trigger' || event.type === 'disaster-reveal' ? event.text") && board.includes('}, 3000)') && !board.includes('reveal-confirm') && !board.includes('public-reveal-mask'), '公开展示卡与触发式效果必须在全局最上层播放三秒无蒙版非阻塞动画；触发动画必须直接使用服务端给出的本次触发效果文本，不得拼接完整卡面或使用确认弹框'],
  ...
  [l12Net.includes("send({ type: 'sandboxAction', actingPlayerIndex, command })") && board.includes('controlledPlayerIndex') && board.includes('const viewMe = computed(() => props.game.players[props.game.you])') && board.includes('const viewEnemy = computed(() => props.game.players[1 - props.game.you])') && board.includes('v-if="l12State.gmEnabled" :cards="viewEnemy.hand"') && board.includes(':cards="viewMe.hand"') && board.includes(':controllable="isControlledPlayer(viewEnemy.playerIndex)"') && prompt.includes('sandboxAction(actingPlayerIndex, command)'), '沙盒必须固定我方在下、对方在上，不交换棋盘，同时可查看双方手牌并代行双方规则内选择'],

...
const contracts = [
  ...
  [actionPresentation.includes("if (event.type !== 'phase') return null") && actionPresentation.includes("kind: 'phase'") && !actionPresentation.includes("turn-start") && !actionPresentation.includes("event.type === 'play'") && !actionPresentation.includes("event.type === 'attack'"), '通用动作条只能呈现阶段变化，不得承载回合开始、打出、登场或区域移动'],
  [board.includes("import ZoneMovementPresentationLayer from './ZoneMovementPresentationLayer.vue'") && board.includes('<ZoneMovementPresentationLayer :events="game.recentEvents ?? []"') && zoneMovementLayer.includes('data-ui-contract="authoritative-zone-card-movement"'), '打出、登场与区域移动必须由独立卡牌实体跨区域动画承载'],
  ...
  [prompt.includes(":intent=\"isDisasterChoice ? 'detail' : 'thumb'\"") && prompt.split(":alt=\"entry.card.name || '天灾'\" intent=\"detail\"").length - 1 === 2 && prompt.includes("'disaster-choice': isDisasterChoice"), '公开天灾禁选、选择卡及已公开历史必须请求详情级高清图，不得使用缩略图源'],
  ...
  [gamePage.includes("import { gameAction, l12State, leaveRoom, returnToRoom } from './net'") && gamePage.includes("game.value?.phase === 'GameOver'") && gamePage.includes('returnToRoom()') && gamePage.includes('l12State.room?.sandbox'), '正式对局结束必须保留房间并回到重新准备，只有沙盒返回大厅时退出房间'],

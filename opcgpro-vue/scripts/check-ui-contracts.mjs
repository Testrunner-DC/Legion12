...
const contracts = [
  ...
  [decks.includes('export async function loadCardArchiveCatalog()') && decks.includes('productInclusionsByCardId.has(card.cardNo)')
    ...
    && cardArchive.includes('platformState.account ? alternateArtApi.mine()')
  ...
  [visualTransitionProjection.includes("if (from === 'library') return true") && visualTransitionProjection.includes("if (!card || event.type === 'counter-set') return true") && visualTransitionProjection.includes("if (!card.cardId || card.cardId === 'hidden-card') return true") && visualTransitionProjection.includes('return card.hidden === true && card.identityKnown !== true') && zoneMovementLayer.includes('const concealed = isMovementCardConcealed(event, card, from)') && !zoneMovementLayer.includes('const concealed = !card || card.identityKnown === false'), '公开打出的正面卡在全程跨区移动中不得因 identityKnown 默认值回退卡背；普通牌库起点、无卡身份、占位卡或真正未知盖伏卡必须保持正式卡背'],
  ...
  [board.includes('data-ui-contract="complete-player-summary"')
    && board.includes(':rank="enemyBadge?.rank"') && board.includes(':tier-label="battleTierLabel(enemyBadge)"') && board.includes(':placement-title="identityLabel(enemyBadge?.placementTitle)"') && board.includes(':master-title="identityLabel(enemyBadge?.masterTitle)"')
    && board.includes(':rank="myBadge?.rank"') && board.includes(':tier-label="battleTierLabel(myBadge)"') && board.includes(':placement-title="identityLabel(myBadge?.placementTitle)"') && board.includes(':master-title="identityLabel(myBadge?.masterTitle)"')

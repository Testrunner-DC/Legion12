...
try {
  ...
  for (const width of [1440, 1000, 390]) {
    ...
    await page.getByRole('button', { name: '分析卡牌', exact: true }).click()
    ...
    assert.match(await page.locator('.card-heading').innerText(), /上手提升区间跨零，方向未确定/)
    assert.equal(await page.locator('.card-heading em').getAttribute('data-tone'), 'neutral')
    assert.equal(await page.evaluate(() => window.detailCalls[0].mode), 'ranked')
    assert.equal(await page.evaluate(() => window.detailCalls[0].effectVersion), 'current')
    ...
    await page.getByText('样本可靠性', { exact: true }).scrollIntoViewIfNeeded()
    await page.screenshot({ path: path.join(out, 'analytics-detail-' + width + '.png') })
    await page.getByText('数据质量与覆盖', { exact: true }).scrollIntoViewIfNeeded()
    const bottomVisible = await page.getByText('数据质量与覆盖', { exact: true })
  ...
  await empty.getByRole('button', { name: '卡牌数据清单', exact: true }).click()

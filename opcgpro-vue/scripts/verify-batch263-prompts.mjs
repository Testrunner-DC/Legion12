...
try{
 ...
 for(const viewport of [{width:760,height:480},{width:390,height:540}]){
  ...
  assert.equal(await page.locator('.prompt-action-footer').count(),0,'Pure binary effects must not restore a redundant footer in short viewports')
  const actions=await page.locator('.prompt-choices').boundingBox();assert(actions.y+actions.height<=viewport.height+1,'Binary actions remain reachable in short viewport')

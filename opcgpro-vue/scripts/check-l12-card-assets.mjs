...
const starterMoraleVersions = [
  ...
  ['ST05-C1st', 'ST05|奥林匹斯阵营预组', 'S02-05C1A'],
...
const contracts = [
  ...
  [generator.includes("baseUrl = (args.get('--base-url') || '/card-assets'") && generator.includes('expectedPlayableCardCount = 324') && generator.includes('expectedPresentationCardCount = 37'), '生成器必须默认同源内容寻址路径并严格区分324张可玩卡与37张展示版本'],
  ...
  [archiveAssets.cards?.length === 37 && archiveAssets.cards.every(card => ids.has(card.baseCardId)), '37张卡查展示版本必须全部指向现有可玩规则基底'],
  ...
  [windowsDeploy.includes('cardAssetsHash') && serverDeploy.includes('static_card_assets_dir') && serverDeploy.includes('validate_card_assets_tree') && serverDeploy.includes('manifest.cardCount !== 361'), '发布流程必须独立校验并复用完整的内容寻址优化卡图包'],

...
try {
  ...
  for (const width of [1280, 390]) {
    ...
    assert.equal(await page.locator('.metric-switch button').count(), 11, 'all actual daily metric sequences must be switchable')
    assert.equal(await page.locator('.bars article').first().getAttribute('data-zero'), 'true', 'a real zero must remain distinct from missing data')
    await page.getByRole('button', { name: 'PV', exact: true }).click()
    assert.match(await page.locator('.bars').getAttribute('aria-label'), /PV逐日趋势，单位次/)

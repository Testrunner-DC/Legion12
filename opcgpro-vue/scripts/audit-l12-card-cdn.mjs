...
if (manifest.schemaVersion !== 2 || manifest.complete !== true || manifest.cardCount !== expectedCardCount) throw new Error(`发布清单必须是完整的 schema v2 ${expectedCardCount} 张清单`)
...
if (catalog.length !== expectedCardCount || expectedIds.size !== expectedCardCount || entries.length !== expectedCardCount) throw new Error(`权威目录、唯一卡号与资源清单必须同时为 ${expectedCardCount}`)
...
const calculatedVersion = createHash('sha256')
  ...
  .digest('hex')
if (calculatedVersion !== manifest.assetVersion) throw new Error(`assetVersion 与 ${expectedCardCount} 张内容哈希不一致`)
console.log(`L12 优化卡图审计通过：${expectedCardCount}/${expectedCardCount}，${totalBytes} 字节，版本 ${manifest.assetVersion}`)

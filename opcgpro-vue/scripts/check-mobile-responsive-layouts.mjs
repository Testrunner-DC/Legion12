...
expect(viewportCss.includes('height: clamp(128px, 46cqh, 190px) !important') && viewportCss.includes('grid-template-rows: minmax(0, 1fr) minmax(2.5em, max-content)') && viewportCss.includes('flex-basis: clamp(78px, 17cqw, 108px) !important'), 'mobile choice rows must give every orientation and state one fixed peer box with reserved name/state lanes')

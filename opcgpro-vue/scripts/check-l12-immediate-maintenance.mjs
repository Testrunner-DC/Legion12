...
const shell=read('site/SiteShell.vue'),panel=read('site/ImmediateMaintenancePanel.vue'),ticker=read('site/MaintenanceTicker.vue'),admin=read('site/AdminOperationsPanel.vue'),api=read('platform.ts'),net=read('net.ts')
...
assert(ticker.includes('l12State.operationsPolicy?.maintenance')&&!ticker.includes('getEffectiveOperationsPolicy()'))
assert(!ticker.includes('setInterval')&&!ticker.includes('visibilitychange'))
assert(net.includes("message.type === 'effectiveOperationsPolicy'")&&net.includes("dispatchResourceChange('operationsPolicy'"))
assert(ticker.includes('prefers-reduced-motion:reduce')&&ticker.includes('aria-hidden="true"'))
console.log('Immediate maintenance UI guards passed: manual-vs-scheduled rendering, dedicated commands, old backend safety and WebSocket-driven shared policy.')

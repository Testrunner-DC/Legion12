...
const checks = [
  [analyzeSource(profile, 'ProfilePage.vue').maximumParallelPageLoad <= 3 && analyzeSource(admin, 'AdminPage.vue').maximumParallelPageLoad <= 3, '本批后台与个人页 AST 扇出必须不超过三路'],
  ...
  [admin.includes("tab === 'rules'") && adminNavigation.includes("label: '规则中心审核'")

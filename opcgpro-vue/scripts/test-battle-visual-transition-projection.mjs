...
assert.equal(model.isMovementCardConcealed({ ...event, type: 'mill' }, sourceCard, 'library'), true, 'library-origin movement always uses the official card back')
assert.equal(model.isMovementCardConcealed({ ...event, type: 'return' }, sourceCard, 'graveyard'), false, 'public graveyard return keeps its known face during movement')

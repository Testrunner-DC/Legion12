...
const checks = [
  ...
  ['主城、主牌、士气、额外区、备选区独立分区', ['data-deck-section="master"', 'data-deck-section="main"', 'data-deck-section="morale"', 'data-deck-section="extra"', 'data-deck-section="bench"'].every(value => editor.includes(value)) && editor.includes('<span>主城</span>')],
  ['主城候选排除未实装 divinity 模式', editor.includes("card.cardType === 'master'") && !editor.match(/card\.cardType === 'master'\s*\|\|\s*card\.cardType === 'divinity'/)],
  ['已保存牌库只有一个切换入口且详情可折叠', editor.includes('@click="savedDecksOpen = true">切换牌库') && editor.includes('class="saved-decks-dialog"') && editor.includes('detailCollapsed') && !editor.includes('class="saved-deck-switcher"') && !editor.includes('class="saved-decks-panel grand-panel"')],

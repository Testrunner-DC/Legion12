...
const checks = [
  ...
  ['已保存牌库还原到详情下方的唯一列表且详情可折叠', editor.includes('class="saved-decks-panel grand-panel"') && editor.includes('class="saved-list"') && editor.includes('<DeckProfile compact') && editor.includes('detailCollapsed') && !editor.includes('<label>已保存牌库<select') && !editor.includes('class="saved-deck-switcher"')],

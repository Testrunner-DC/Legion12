...
const checks = [
  ['Gallery / Stats / Hand 使用同一编辑器工作区', ['Gallery 卡池', 'Stats 统计', 'Hand 起手'].every(value => editor.includes(value))],
  ['工作区使用 v-show 保留卡池筛选与滚动节点', ['workspace === \'gallery\'', 'workspace === \'stats\'', 'workspace === \'hand\''].every(value => editor.includes(`v-show="${value}"`))],
  ['起手从未保存的当前构筑抽取且不写后台', editor.includes('samplePublicDeckOpeningHand(entries.value.flatMap') && editor.includes('不会修改牌库或生成对局记录')],
  ['移动端为卡池、牌表、统计/起手单任务入口', ['setMobilePane(\'pool\')', 'setMobilePane(\'deck\')', 'setMobilePane(\'insights\')'].every(value => editor.includes(value))],
  ['移动端筛选使用底部面板并显示启用数量', editor.includes('mobileFiltersOpen') && editor.includes('activeFilterCount') && editor.includes('mobile-filter-trigger')],
  ...
  ['主宰、主牌、士气、额外区、备选区独立分区', ['data-deck-section="master"', 'data-deck-section="main"', 'data-deck-section="morale"', 'data-deck-section="extra"', 'data-deck-section="bench"'].every(value => editor.includes(value))],

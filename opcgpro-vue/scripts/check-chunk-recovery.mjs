...
assert.match(routerSource, /router\.onError\(\(error, to\) => \{ handleChunkLoadError\(error, 'router', to\.fullPath\) \}\)/)
...
console.log(`[chunk recovery] ${chunkFailures.length} chunk failures, ${ordinaryErrors.length} ordinary errors, one-shot wiring passed.`)

...
const { pending: actionBusy, run: runAction } = useActionGate()
const notice = ref('')
...
async function add(player: PlatformFriend) {
  await runAction('friends-write', async () => {
...
<template>
  <div class="friends-page" :aria-busy="actionBusy">
    ...
    <section v-if="tab === 'friends'" class="panel friend-center">
      ...
      <main v-if="selected"><div class="hero-avatar">{{ selected.username.slice(0, 1) }}</div><h2>{{ selected.username }}</h2><p>{{ selectedPresence?.online ? selectedPresence.activity === 'playing' ? '正在对局' : '当前在线' : '当前离线' }}</p><div><button v-if="selectedPresence?.canInvite" :disabled="actionBusy" @click="invite(selected)">邀请对战</button><button v-else-if="selectedPresence?.canSpectate" :disabled="actionBusy" @click="spectate(selected)">进入观战</button><button v-else disabled>{{ selectedPresence?.actionReason || '暂不可操作' }}</button><button class="quiet" :disabled="actionBusy" @click="remove(selected)">删除好友</button><button class="danger" :disabled="actionBusy" @click="blockPlayer(selected)">屏蔽</button></div></main>
      <div v-else class="empty">选择一位好友<br/>查看状态并发起对战。</div>
    </section>
    <section v-else-if="tab === 'requests'" class="panel request-panel">
      <h2>收到的申请</h2><article v-for="player in incoming" :key="player.accountId"><b>{{ player.username }}</b><div><button class="danger" :disabled="actionBusy" @click="blockPlayer(player)">屏蔽</button><button class="quiet" :disabled="actionBusy" @click="resolve(player, false)">拒绝</button><button :disabled="actionBusy" @click="resolve(player, true)">接受</button></div></article><p v-if="!incoming.length" class="empty compact">暂无待处理申请</p>

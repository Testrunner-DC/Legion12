...
async function loadAccountData() {
  ...
  const [matchResult, sessionResult, rankedResult, statisticsResult] = await Promise.allSettled([
    platformRequest<Match[]>('/api/matches?limit=10'), sessionApi.list(), rankedApi.overview(), playerApi.statistics(),
  ])
...
const authSubmitLabel = computed(() => authBusy.value
  ...
  : (authMode.value === 'login' ? '登录' : '建立账号'))
function selectAuthMode(mode: 'login' | 'register') {
...
function authenticationFailureMessage(error: unknown) {
  if (error instanceof PlatformRequestError) {
    // 登录和注册端点不会以 403 表示凭据错误。此状态通常代表浏览器仍连接到已失效的站点节点。
    if (error.status === 403)
      return '当前连接的站点节点已失效，请彻底关闭浏览器中的所有十二军团页面，再重新打开网站后重试。'
    // 401 与 429 使用服务端统一文案，既保留限流提示，也不泄露账号是否存在。
    if (error.status === 401 || error.status === 429) return error.message
    return error.message || '账号请求未完成，请稍后重试'
  }
  return '暂时无法连接账号服务，请检查网络后重试'
}
async function submitAuth() {
  if (authBusy.value) return
  authBusy.value = true; notice.value = ''; authNotice.value = ''
  ...
  try {
    ...
    if (authenticationCompleted || platformState.account)
...
async function signOut() {
  ...
  finally { sessions.value = []; renameStatus.value = null; playerStatistics.value = null; ranked.value = null; matches.value = []; authBusy.value = false }
}
...
<template>
  <div class="profile-page">
    ...
    <section v-if="playerStatistics" class="performance-panel"><header><div><small>PLAYER PERFORMANCE</small><h2>详细战绩</h2></div><span>统计摘要独立保留，不依赖录像文件</span></header><div class="side-stats"><article><span>先手</span><b>{{ playerStatistics.overall.firstGames }} 场 · {{ sideWinRate(playerStatistics.overall.firstWins, playerStatistics.overall.firstGames) }}</b></article><article><span>后手</span><b>{{ playerStatistics.overall.secondGames }} 场 · {{ sideWinRate(playerStatistics.overall.secondWins, playerStatistics.overall.secondGames) }}</b></article><article><span>排位胜率</span><b>{{ winRate(playerStatistics.ranked) }}</b></article><article><span>最近统计</span><b>{{ playerStatistics.updatedAt ? new Date(playerStatistics.updatedAt).toLocaleDateString() : '暂无' }}</b></article></div><div class="master-records"><header><b>各主宰战绩</b><span>同时列出整体与排位表现</span></header><article v-for="master in playerStatistics.masters" :key="master.masterId"><div><b>{{ master.masterName }}</b><small>{{ master.masterId }}</small></div><span>整体 {{ master.overall.wins }}胜 {{ master.overall.losses }}负 {{ master.overall.draws }}平 · {{ winRate(master.overall) }}</span><span>排位 {{ master.ranked.games }} 场 · {{ winRate(master.ranked) }}</span><span>先手 {{ sideWinRate(master.overall.firstWins, master.overall.firstGames) }} / 后手 {{ sideWinRate(master.overall.secondWins, master.overall.secondGames) }}</span></article><p v-if="!playerStatistics.masters.length">完成包含主宰快照的对局后，这里会按主宰分别统计。</p></div></section>
...
.performance-panel{margin:0 0 12px;padding:18px;border:1px solid #36525a;background:#0d171d}.performance-panel>header,.master-records>header{display:flex;align-items:flex-end;justify-content:space-between;gap:12px}.performance-panel h2{margin:4px 0 0}.performance-panel>header small{color:#55c6cd;font-size:12px;font-weight:900;letter-spacing:.12em}.performance-panel>header span,.master-records>header span{color:#77868b;font-size:12px}.side-stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-top:12px}.side-stats article{padding:11px;border:1px solid #304047;background:#091116}.side-stats span,.side-stats b{display:block}.side-stats span{color:#7f8e93;font-size:12px}.side-stats b{margin-top:4px;font-size:14px}.master-records{margin-top:14px;padding-top:13px;border-top:1px solid #304047}.master-records article{display:grid;grid-template-columns:minmax(150px,1fr) repeat(3,minmax(150px,1fr));align-items:center;gap:10px;margin-top:7px;padding:10px;border:1px solid #293940;background:#0a1217}.master-records article div{display:grid;gap:2px}.master-records article small,.master-records article span{color:#8d999d;font-size:12px}.master-records>p{color:#7f8c91;font-size:12px}@media(max-width:820px){.side-stats{grid-template-columns:1fr 1fr}.master-records article{grid-template-columns:1fr 1fr}.master-records article div{grid-column:1/-1}}@media(max-width:520px){.performance-panel{padding:13px}.performance-panel>header{align-items:flex-start;flex-direction:column}.master-records article{grid-template-columns:1fr}.master-records article div{grid-column:auto}}

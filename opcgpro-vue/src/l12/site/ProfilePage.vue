...
<template>
  <div class="profile-page">
    ...
    <section v-if="ranked" class="rank-overview"><header><div><small>RANKED PROFILE</small><h2>本赛季排位</h2></div><div class="rank-links"><router-link to="/battle/rankings">查看排行榜 →</router-link></div></header><div class="rank-body"><article><span>派系</span><b>{{ ranked.profile.faction || '尚未选择' }}</b></article><article><span>段位</span><b>{{ ranked.profile.rankLabel }}</b></article><article><span>七曜值</span><b>{{ ranked.profile.sevenValue.toLocaleString() }}</b></article><article><span>排位胜率</span><b>{{ rankedWinRate }}</b></article></div><div v-if="ranked.profile.titles.length" class="profile-titles"><span v-for="title in ranked.profile.titles" :key="title">✦ {{ title }}</span></div><p v-else>达到称号条件后会在这里展示派系与最强主宰称号。</p><section class="title-manager"><div><b>最强称号管理</b><button type="button" @click="masterTitleRulesOpen = true">最强称号规则</button><span>对战中默认显示段位和1个最强主宰称号；若位列本派系第1至5名，则派系排名称号代替段位。</span></div><template v-if="ranked.profile.masterTitles.length"><select v-model="selectedMasterTitle"><option v-for="title in ranked.profile.masterTitles" :key="title" :value="title">{{ title }}</option></select><button :disabled="authBusy || selectedMasterTitle === (ranked.profile.selectedMasterTitle || '')" @click="saveRankedTitle">保存称号</button></template><em v-else>近 30 日尚未获得最强主宰称号</em></section></section>

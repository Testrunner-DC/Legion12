      ...
      <section class="match-detail panel-shell">
        ...
        <template v-else-if="detail">
          ...
          <section class="fact-timeline">
            ...
            <ol v-if="detail.cardFacts?.length"><PagedCollection :items="detail.cardFacts.slice(-120)" v-slot="{ items: paged16000 }"><li v-for="fact in paged16000" :key="`${fact.commandSequence}-${fact.kind}-${fact.cardId}-${fact.cardInstanceId}`"><code>#{{ fact.commandSequence }}</code><span><b>{{ factLabel(fact.kind) }}</b><small>回合 {{ fact.round ?? '—' }} · {{ fact.phase || '—' }}</small></span><span v-if="fact.cardId || fact.relatedCardId"><CardImage :card-id="fact.cardId || fact.relatedCardId || ''" :alt="cardById.get(fact.cardId || fact.relatedCardId || '')?.nameZh || fact.cardId || fact.relatedCardId || '卡牌'" intent="thumb"/><em>{{ cardById.get(fact.cardId || fact.relatedCardId || '')?.nameZh || fact.cardId || fact.relatedCardId }}</em></span><strong v-if="fact.amount">{{ fact.amount > 0 ? '+' : '' }}{{ fact.amount }}</strong></li></PagedCollection></ol>

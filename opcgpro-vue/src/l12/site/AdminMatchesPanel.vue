        ...
        <template v-else-if="detail">
          ...
          <section class="participant-grid">
            <article v-for="participant in detail.participants" :key="participant.playerIndex" class="participant-card">
              ...
              <button v-if="participant.deckCards?.length" class="view-construction" data-ui-contract="match-snapshot-view-construction" @click="deckViewer = participant">查看构筑 <small>{{ participant.deckCards.reduce((sum, card) => sum + card.quantity, 0) }} 张快照</small></button>

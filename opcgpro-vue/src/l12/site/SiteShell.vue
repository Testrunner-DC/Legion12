...
const updateEntries = [
  {
    ...
    sections: [
      ...
      { title: '对战操作与玩家信息', items: [
        ...
        '防守与支援提示只保留“确认抵挡／不抵挡”和“确认支援／不支援”等玩家操作；目标高亮只落在本次真正被进攻或被选择的对象，不再让其他战场位置同时亮起。',
      ] },
      ...
      { title: '公开牌库与构筑内容', items: [
        ...
        '公开内容保存后可再次编辑；所有公开版本和逐卡变化长期保留。对局只在开局时已有明确公开编号、版本和构筑哈希时才归入对应版本，后来公开、复制构筑或旧记录不会被猜测关联。',
        '版本对局会显示真实胜负、对方主宰与开局版本；作废对局、统计排除账号和完整性复核中的对局会同步排除。回放已过期时仍保留赛果，但不再显示不可用的回放入口。',
      ] },
      { title: '异画、画廊与获得提示', items: [
        '画廊只展示异画；新上传并公开的异画会使用独立编号和所绑定原卡出现在画廊。图鉴仍保持原有浏览方式，不因移动端优化一次加载全部大图。',
        '异画到账提示精简为“获得异画！”，展示卡名、异画编号和管理员填写的原因；玩家确认后仍可在“我的”收藏中查看。',
      ...
      { title: '对局记录与回放', items: [
        '对局记录改为玩家可读的结果投影：费用、目标、资源、手牌、试炼、区域移动、天灾和额外回合按实际发生结果合并展示，隐藏响应栈、失败分支和服务端内部术语。',
      ...
      { title: '卡牌结算与规则一致性', items: [
        '全卡池的发动声明、费用支付、目标选择、响应、无效、结算、恢复和对局记录统一使用同一生命周期；重连或重复提交不会跳过费用、改选目标或重复结算。',
        '墨子、拉美西斯、安卡神碑、卡诺匹斯罐一／四、安倍晴明、稻姬、井伊直虎等登场效果，以及阿伊、土方岁三、高杉晋作、月读、迦具土、战略转移、神妙行军、万箭齐发和伪造密令等目标效果，会在响应结束后重新确认原对象仍在正确区域、阵营和类型；对象失效时不偷偷改选。',
        '刘备、西施、锡瓦的卡巴、沙漠君临、防御部署与上杉谦信等从手牌登场的流程，会逐张确认原手牌和原位置；位置被占、对象离开或效果被无效时按各自费用与去向规则处理，不用其他卡补位。',
        '佣兵部队、绝对防御、空城计、战斗至黎明及保护型反击统一检查本次进攻与响应对象；必中、不能进攻、不能支援、骑兵移动和无法因效果转为活跃等限制，在按钮、提交、结算和恢复时保持一致。',
        '免死效果会清除本次伤害并把当前兵力设为1000；《猎杀时刻》《图特摩斯三世》及相关墓地、固定数量回库和试炼流程补齐恢复与对象失效检查。固定要求N张的效果在数量不足时不会只处理一部分。',
      ] },

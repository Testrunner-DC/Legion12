...
const updateEntries = [
  {
    ...
    sections: [
      { title: '移动端横屏与对局操作', items: [
        ...
        '士气在横屏时可点开大尺寸选择面板，仍保留原位置的实时数量；点选手牌或场上军团后，可执行操作会集中显示在结束回合按钮上方。对局记录改为可展开查看，避免挤占战场。',
        '进攻后的选择只显示“确认抵挡／不抵挡”或“确认支援／不支援”。战斗信息使用“进攻值”等玩家可读名称，不再显示内部状态词。',
      ] },
      { title: '天灾与卡面呈现', items: [
        '没有额外触发效果的天灾公开时，也会从牌背翻出对应卡面。展开中的选择、墓地、主宰、记录或士气面板会让尚未开始的卡面动画等待；已经开始的动画会自然结束，不会在关闭面板后重复播放。',
        '天灾因回合阶段自然触发和因军团或卡牌效果主动触发会按各自规则结算；《诸神黄昏》的开场与主动分支不再混用。',
      ] },
      { title: '卡牌效果与结算', items: [
        '《贝奥武夫》《古斯塔夫一世》《奥德修斯》《彭忒西勒亚》《斯巴达勇士》《帕西瓦尔》的进攻时兵力提升，及《尼托克丽丝》《神箭奥德尔》《疯狂的爱丽丝》《安提诺乌斯》的单目标状态效果，都会按结算时仍合法的对象处理；被无效、对象失效或没有对象会显示正确结果。',
        '《梅林》《帕西瓦尔》《高文》《阿麦金》《伊丽莎白·都铎》《加雷斯》《纯白的灵鹿》登场获得符文，以及九项获得或翻转士气、符文的触发，统一按实际发生的结果结算。试炼完成后的翻面先作为规则行动执行，翻面后触发的效果再单独进入响应。',
        ...
        '《野外扎营》《山河社稷图》《观星》《法老王的庆典》《众神之乡》《无骨者伊瓦尔》《花魁的馈赠》《武运在天 铠甲在前》《柏拉图》《普罗米修斯》《符文之力》《特勒马科斯》的牌库顶查看、选择和回顶／回底会在重连后保持可继续操作；非法或过期选择不会让对局卡住。检索牌实际加入手牌后保留原有的响应窗口，首次提示不会提前公开隐藏牌。',
        '明确写有次数限制的能力会按卡面规则记录次数；《信仰狂热者》拒绝发动不占用次数，《密米尔之泉》等同名限制会正确共享。远程、骑兵位移、后排主动与主宰次数也会按当前来源、位置和卡面条件判断。',
      ] },
      { title: '规则中心与卡牌收藏', items: [
        '规则中心改为按基础规则、对局流程、关键词、常见问答、单卡裁定和版本记录浏览；内容以已发布规则资料为准，并可按关键词检索。',
        '获得使用权的异画可在牌库编辑时为对应卡牌选择。异画只改变本人对局中看到的卡图，不改变卡牌规则、牌库合法性或其他玩家的显示。',
      ] },
      { title: '账号与个人页面', items: [
        '“我的”页面现在可直接完成一次昵称修改；再次修改时可填写原因提交申请，结果会在个人页面显示。',
      ] },
    ],
  },
  {

  ...
  <section class="construction-browser" data-ui-contract="shared-deck-construction-browser">
    ...
    <div class="construction-workspace">
      <div class="construction-grid">
        <button v-for="entry in visible" :key="`${entry.section}-${entry.cardId}`" :class="{ selected: selectedId === entry.cardId }" @click="selectedId = entry.cardId">
      ...
      <aside v-if="selected"><CardImage :card-id="selected.id" :legacy-url="selected.imageUrl" :alt="selected.nameZh" intent="detail" eager/><small>{{ selected.number }}</small><h4>{{ selected.nameZh }}</h4><p>{{ cardTypeLabel(selected.cardType, selected.isCounterTactic) }} · {{ selected.faction }}</p><div>{{ selected.effect || '无效果文字' }}</div></aside>
    </div>
  </section>
</template>

...
const centerItems = computed(() => {
  try {
    const document = JSON.parse(ruleCenterDraft.value) as Record<string, Array<{ id?: string; title?: string; status?: string }>>
    return ['coreBlocks', 'quickStart', 'terms', 'tournament', 'versions'].flatMap(collection =>
      (Array.isArray(document[collection]) ? document[collection] : []).filter(item => item.id)
        .map(item => ({ collection, id: item.id!, title: item.title || (item as { topic?: string }).topic || item.id!, status: item.status || 'pending' })))
  } catch { return [] }
})
...
async function load() {
  ...
  try {
    ...
    ruleCenterDraft.value = JSON.stringify(centerDocument, null, 2)
  }
...
async function publishRuling(item: RuleRuling) {
  ...
  try {
    ...
    await adminApi.saveContentDraft('rules.rulings', JSON.stringify({ entries: entries.value }, null, 2))
    await adminApi.publishRuleItem('rules.rulings', 'entries', item.id)
...
async function publishCenterItem(collection: string, itemId: string) {
  ...
  try {
    await adminApi.saveContentDraft('rules.center', ruleCenterDraft.value)
    await adminApi.publishRuleItem('rules.center', collection, itemId)
    const document = JSON.parse(ruleCenterDraft.value) as Record<string, Array<Record<string, unknown>>>
    const item = document[collection]?.find(row => row.id === itemId)
    if (item) item.status = 'published'
    ruleCenterDraft.value = JSON.stringify(document, null, 2)
...
<template>
  <section class="ruling-admin">
    <header><div><small>RULE CENTER REVIEW</small><h3>规则中心与 FAQ 裁定库</h3><p>每条内容独立审核、独立发布。公开页只读取已发布条目的快照，发布一条不会连带公开其他草稿。</p></div><div class="actions"><button @click="load">{{ busy ? '读取中…' : '刷新' }}</button><button v-if="hasPermission('admin.content.draft')" @click="addRuling()">＋ 新建裁定</button><button v-if="hasPermission('admin.content.draft')" @click="save()">保存草稿</button><button @click="preview">预览草稿差异</button></div></header>
    <details class="rule-center-document"><summary>规则中心资料草稿（核心规则、快速入门、术语、赛事与版本记录）</summary><p>编辑后先保存草稿，再从下方逐项审核发布。不得使用会与卡牌规则混淆的程序术语。</p><textarea v-model="ruleCenterDraft" rows="18" spellcheck="false" aria-label="规则中心资料 JSON 草稿"></textarea><div class="center-review-list"><article v-for="item in centerItems" :key="`${item.collection}-${item.id}`"><div><b>{{ item.title }}</b><small>{{ item.id }} · {{ item.status === 'pending' ? '待审核' : '草稿标记为可发布' }}</small></div><button v-if="hasPermission('admin.content.publish')" class="publish" :disabled="publishingId === item.id" @click="publishCenterItem(item.collection, item.id)">{{ publishingId === item.id ? '发布中…' : '审核并发布此项' }}</button></article></div></details>
...
.center-review-list,.publish-queue{display:grid;gap:8px;margin-top:12px}.center-review-list article,.publish-queue article{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px;border:1px solid #344149;background:#0d151b}.center-review-list article div,.publish-queue article div{display:grid;gap:3px}.center-review-list small,.publish-queue small{letter-spacing:0}.publish-queue{margin-top:18px;padding:14px;border:1px solid #4b4731;background:#10140e}.publish-queue h4,.publish-queue p{margin:0}
</style>

...
const status = ref<ServerStorageStatus | null>(null); const loading = ref(false)
...
const trendSummary = computed(() => {
  ...
  const first = points[0]; const last = points[points.length - 1]; const mounts = new Set([...Object.keys(first.volumeUsedPercent), ...Object.keys(last.volumeUsedPercent)])
  let largest = { mount: '', delta: 0 }
  mounts.forEach(mount => { const delta = (last.volumeUsedPercent[mount] ?? 0) - (first.volumeUsedPercent[mount] ?? 0); if (Math.abs(delta) > Math.abs(largest.delta)) largest = { mount, delta } })
...
async function load() { loading.value = true; try { status.value = await adminApi.serverStorage() } catch (error) { emit('notice', error instanceof Error ? error.message : '服务器存储状态读取失败') } finally { loading.value = false } }
onMounted(load)
</script>
<template>
  <section class="storage-panel">
    ...
    <template v-if="status">
      <section class="health-summary" :data-health="status.health"><small>当前结论</small><h3>{{ status.conclusion }}</h3><p>{{ status.impact }}</p><b>{{ status.recommendedAction }}</b><span class="freshness" :data-freshness="freshness.state">采样新鲜度：{{ freshness.label }}</span><span>权威阈值：关注 {{ status.thresholds.warningPercent }}% · 紧急 {{ status.thresholds.criticalPercent }}% · 来源：{{ status.thresholds.source }}</span></section>

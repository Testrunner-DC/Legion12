...
async function savePresentation(scene: EffectPresentationScene) {
  ...
  try {
    ...
    notice.value = `${scene.label} 的动效文案已保存并写入审计；仅新建对局使用新文案`
...
async function restorePresentation(scene: EffectPresentationScene) {
  ...
  try {
    ...
    await refreshSelectedEffect(selectedEffect.value.cardId)
    notice.value = `${scene.label} 已恢复默认文案并写入审计；进行中对局不受影响`
          ...
          <div v-if="showDeletedAccounts" class="deleted-accounts-overlay" @click.self="showDeletedAccounts = false">
            <section class="deleted-accounts-dialog" role="dialog" aria-modal="true" aria-label="已删除账号">
              ...
              <div class="deleted-account-list">
                <article v-for="account in deletedAccounts" :key="account.id">
          ...
          <section class="panel effect-detail">
            <template v-if="selectedEffect">
              ...
              <article v-for="ability in selectedEffect.abilities" :key="ability.abilityId" class="ability-card">
                ...
                <section v-if="ability.presentations?.length" class="presentation-editor" data-ui-contract="effect-presentation-editor">
                  <header><div><b>对战卡牌动效文案</b><small>独立于规则参数与卡面原文；保存后只影响新建对局，换行会原样展示。</small></div></header>
                  <article v-for="scene in ability.presentations ?? []" :key="scene.sceneId" class="presentation-scene">
                    <div class="presentation-heading">
                      <span><b>{{ scene.label }}</b><code>{{ scene.eventType }} · {{ scene.trigger }}</code></span>
                    ...
                    <label>编辑文案<textarea v-model="presentationDrafts[scene.sceneId]" rows="4" maxlength="2000" :aria-label="`${selectedEffect.name} ${scene.label} 动效文案`"/></label>

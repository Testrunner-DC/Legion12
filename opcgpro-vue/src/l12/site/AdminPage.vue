  ...
  <div class="admin-page">
    ...
    <template v-else>
      ...
      <section v-else-if="tab === 'security'" class="security-workbench">
        ...
        <section class="panel security-boundaries"><header><h2>发布恢复与账号保护</h2></header><article><b>发布复核离线恢复</b><span>{{ securityStatus?.offlineBootstrapUsed ? '一次性恢复已使用' : securityStatus?.offlineBootstrapEnabled && securityStatus?.offlineBootstrapCredentialConfigured ? '离线恢复入口已显式启用' : '默认关闭或凭据未配置' }}</span><p>仅服务器 CLI 可用，只服务受控发布恢复；账号与赛事不需要另一人审批。</p></article><article><b>MFA</b><span>{{ securityStatus?.mfa.enrollmentEnabled ? '已启用' : '未启用' }}</span></article></section>
...
.bug-diagnostics{margin-top:12px;padding:10px;border:1px solid #34444d;background:#091116}.bug-diagnostics summary{cursor:pointer;color:#83d5e4;font-size:10px;font-weight:900}.bug-diagnostics dl{display:grid;grid-template-columns:110px minmax(0,1fr);gap:5px 10px;margin:9px 0 0;font-size:9px}.bug-diagnostics dt{color:#718087}.bug-diagnostics dd{margin:0;color:#b8c4c6;overflow-wrap:anywhere}

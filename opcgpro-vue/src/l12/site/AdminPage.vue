...
const tab = ref<'bugs' | 'accounts' | 'content' | 'effects' | 'releases' | 'commands' | 'audit' | 'security' | 'operations'>('bugs')
...
async function setRole(account: PlatformAccount) { try { const result = await adminApi.setRole(account.id, account.role); notice.value = `${account.username} 的角色变更已提交审批（命令 ${result.commandId}）`; await loadControlPlane(); await loadAccounts() } catch (error) { notice.value = error instanceof Error ? error.message : '更新失败' } }
...
async function setAccountStatus(account: PlatformAccount) {
  ...
  try {
    if (!securityStatus.value) await loadSecurity()
    if (!securityStatus.value) { notice.value = '安全状态版本不可用，请刷新'; return }
    const result = await adminApi.setAccountStatus(account.id, !account.disabled, reason, securityStatus.value.platformVersion)
    notice.value = 'commandId' in result
      ? `${account.disabled ? '启用' : '禁用'}账号已提交双人审批（命令 ${result.commandId}）`
      : '账号状态 dry-run 已完成'
...
async function loadControlPlane() {
  ...
  } catch (error) { notice.value = error instanceof Error ? error.message : '命令与审批加载失败' }
}
...
onMounted(() => {
  ...
  if (hasPermission('admin.commands.read') || hasPermission('admin.approvals.read')) loadControlPlane()
  if (hasPermission('releases.read') || hasPermission('releases.runtime.read')) loadReleases()
  if (!hasPermission('admin.bugs.read')) tab.value = hasPermission('releases.read') || hasPermission('releases.runtime.read') ? 'releases' : hasPermission('admin.content.read') ? 'content' : 'effects'
})
</script>
...
<template>
  <div class="admin-page">
    ...
    <template v-else>
      <nav>
        <button v-if="hasPermission('admin.bugs.read')" :class="{ active: tab === 'bugs' }" @click="tab = 'bugs'; loadBugs()">Bug 管理</button>
        <button v-if="hasPermission('admin.accounts.read')" :class="{ active: tab === 'accounts' }" @click="tab = 'accounts'; loadAccounts()">账号权限</button>
        <button v-if="hasPermission('admin.content.read')" :class="{ active: tab === 'content' }" @click="tab = 'content'; loadContent()">官网内容</button>
        <button v-if="hasPermission('admin.effects.read')" :class="{ active: tab === 'effects' }" @click="tab = 'effects'; loadEffects()">卡效原子化</button>
        <button v-if="hasPermission('releases.read') || hasPermission('releases.runtime.read')" :class="{ active: tab === 'releases' }" @click="tab = 'releases'; loadReleases()">发布控制</button>
        <button v-if="hasPermission('admin.commands.read') || hasPermission('admin.approvals.read')" :class="{ active: tab === 'commands' }" @click="tab = 'commands'; loadControlPlane()">命令与审批</button>
        <button v-if="hasPermission('admin.security.read')" :class="{ active: tab === 'security' }" @click="tab = 'security'; loadSecurity()">安全治理</button>
        <button v-if="hasPermission('admin.audit.read')" :class="{ active: tab === 'audit' }" @click="tab = 'audit'; loadAudit()">审计日志</button>
      ...
      <section v-else-if="tab === 'accounts'" class="panel"><header><div><h2>账号、权限与会话</h2><p>禁用/启用属于高风险命令，提交后由另一审批人执行；禁用会立即撤销旧令牌与 WebSocket。</p></div><button @click="loadAccounts(); loadSecurity()">刷新</button></header><div class="account-row head"><b>用户名 / 状态</b><span>建立时间</span><span>角色</span><span>有效权限</span><span>操作</span></div><div v-for="account in accounts" :key="account.id" class="account-row"><b>{{ account.username }}<small :data-disabled="account.disabled">{{ account.disabled ? '已禁用' : '正常' }}<template v-if="account.disabledReason"> · {{ account.disabledReason }}</template></small></b><span>{{ new Date(account.createdAt).toLocaleString() }}</span><select v-model="account.role" :disabled="account.username === 'Admin'"><option value="player">玩家</option><option value="support">客服支持</option><option value="referee">裁判</option><option value="organizer">主办者</option><option value="editor">内容编辑</option><option value="release-manager">发布管理员</option><option value="admin">管理员</option></select><small :title="account.permissions?.join('\n')">{{ account.permissions?.length ?? 0 }} 项</small><span class="account-actions"><input v-if="hasPermission('admin.accounts.status.write')" v-model="accountStatusReasons[account.id]" placeholder="状态变更理由"/><button :disabled="account.username === 'Admin'" @click="setRole(account)">保存角色</button><button v-if="hasPermission('admin.accounts.status.write')" class="status" :data-disabled="account.disabled" :disabled="account.username === 'Admin' || account.id === platformState.account?.id" @click="setAccountStatus(account)">{{ account.disabled ? '提交启用审批' : '提交禁用审批' }}</button><button class="revoke" @click="revokeAccountSessions(account)">撤销会话</button></span></div></section>
      ...
      <section v-else-if="tab === 'commands'" class="command-workbench">
        <section v-if="hasPermission('admin.approvals.read')" class="panel approval-panel"><header><div><h2>待审批</h2><p>申请人与审批人必须不同；执行复用原始载荷和版本前置条件。</p></div><button @click="loadControlPlane">刷新</button></header><article v-for="approval in approvals" :key="approval.commandId" class="approval-row"><span><code>{{ approval.commandId }}</code><b>{{ approval.requesterName }}</b><small>{{ new Date(approval.requestedAt).toLocaleString() }}</small></span><input v-model="approvalReasons[approval.commandId]" placeholder="审批理由"/><button @click="showCommand(approval.commandId)">详情</button><button :disabled="approval.requesterId === platformState.account?.id" class="confirm" @click="reviewCommand(approval.commandId, 'approve')">批准并执行</button><button :disabled="approval.requesterId === platformState.account?.id" class="reject" @click="reviewCommand(approval.commandId, 'reject')">拒绝</button></article><div v-if="!approvals.length" class="empty">暂无待审批命令</div></section>
      ...
      <section v-else-if="tab === 'security'" class="security-workbench">
        <section class="panel security-summary"><header><div><h2>安全治理快照</h2><p>高风险操作在独立审计不可用时失败关闭；页面只展示状态，不提供公网 bootstrap 或密钥入口。</p></div><button @click="loadSecurity">刷新</button></header><div v-if="securityStatus" class="security-metrics"><article><small>第二审批人</small><b>{{ securityStatus.secondApproverReady ? '就绪' : '缺失' }}</b><span>{{ securityStatus.activeApprovers }} 名有效审批人</span></article><article><small>高风险审计</small><b>{{ securityStatus.highRiskAuditAvailable ? '可用' : '失败关闭' }}</b><span>保留 {{ securityStatus.auditRetentionDays }} 天</span></article><article><small>账号 / 锁定</small><b>{{ securityStatus.disabledAccounts }} / {{ securityStatus.activeLoginLocks }}</b><span>禁用账号 / 登录锁定</span></article><article><small>待审批</small><b>{{ securityStatus.pendingApprovals }}</b><span>平台版本 v{{ securityStatus.platformVersion }}</span></article></div><div v-if="securityStatus?.alerts.length" class="security-alerts"><article v-for="alert in securityStatus.alerts" :key="alert.code" :data-severity="alert.severity"><b>{{ alert.code }} · {{ alert.count }}</b><span>{{ alert.message }}</span></article></div></section>
        <section class="panel security-boundaries"><header><h2>受控恢复与 MFA 边界</h2></header><article><b>第二审批人离线 bootstrap</b><span>{{ securityStatus?.offlineBootstrapUsed ? '一次性引导已使用' : securityStatus?.offlineBootstrapEnabled && securityStatus?.offlineBootstrapCredentialConfigured ? '离线命令入口已显式启用' : '默认关闭或凭据未配置' }}</span><p>仅服务器 CLI 可用；Web/API 不接受 bootstrap 凭据。</p></article><article><b>MFA</b><span>{{ securityStatus?.mfa.enrollmentEnabled ? '已启用' : '尚未启用' }} · 保护器 {{ securityStatus?.mfa.mode || 'unavailable' }}</span><p>{{ securityStatus?.mfa.requirement || '加载中' }}</p><p>当前不会写入 TOTP 明文或伪造恢复码能力。</p></article></section>
...
@media(max-width:1300px){.effects-layout,.command-workbench,.release-workbench,.security-workbench{grid-template-columns:1fr}.coverage-strip{grid-template-columns:repeat(3,1fr)}.release-compose,.security-summary,.security-archive{grid-column:auto}.release-form{grid-template-columns:1fr 1fr}.account-actions{grid-template-columns:1fr 1fr}}@media(max-width:850px){.bug-row{grid-template-columns:1fr}.account-row{grid-template-columns:1fr 1fr}.operation-grid{grid-template-columns:1fr}.admin-page>nav{overflow-x:auto}.admin-page>nav button{flex:none}.coverage-strip,.release-environments,.security-metrics{grid-template-columns:1fr 1fr}.effect-filters{grid-template-columns:1fr 1fr}.effect-table-head,.effect-row{grid-template-columns:minmax(180px,1fr) 110px}.effect-table-head span:last-child,.effect-row>.status-pill{display:none}.approval-row,.command-row,.content-preview>span,.release-form,.release-artifacts article,.release-runs article,.archive-form,.archive-row{grid-template-columns:1fr}.audit-filters input{width:100%}}
</style>

...
import { adminApi, isAdmin, type AtomicCardEffect, type AtomicCoverage, type BugReport, type EffectAtomDescriptor, type PlatformAccount } from '@/l12/platform'
        ...
        <div class="effects-layout">
          <section class="panel effects-list">
            ...
            <button v-for="card in effectCards" :key="card.cardId" class="effect-row" :class="{ selected: selectedEffect?.cardId === card.cardId }" @click="selectEffect(card)">
              ...
              <span class="effect-count"><b>{{ card.abilities.length }}</b> 能力 / <b>{{ card.atomCount }}</b> 原子<small v-if="card.legacyAtomCount">{{ card.legacyAtomCount }} 个兜底节点</small></span>

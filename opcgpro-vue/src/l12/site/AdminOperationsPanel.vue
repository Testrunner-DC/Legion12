    ...
    <section v-if="activeSection !== 'versions'" class="panel config-panel">
      ...
      <div class="config-grid section-grid">
        <template v-if="activeSection === 'ranked' && rankedConfig">
          ...
          <fieldset class="wide"><legend>五段位共同规则</legend><p class="wide field-help">同等级共用门槛与结算数值，仅段位名称分派系填写。只有第五段位玩家可获得派系排名称号。</p>
            ...
            <div class="wide ranked-shared-tiers"><article v-for="(tier, index) in rankedConfig.factions[0]?.tiers" :key="index">
              ...
              <div class="tier-number-fields"><label v-for="field in sharedTierFields" :key="field.key">{{ field.label }}<input :value="tier[field.key]" type="number" min="0" step="1" :disabled="tierMismatch" @input="updateTier(index, field.key, $event)"/></label></div>
            </article></div>
          </fieldset>

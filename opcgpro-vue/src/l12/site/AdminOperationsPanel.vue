...
const placementMaximumLimit = computed(() => Math.max(0, (rankedConfig.value?.factions[0]?.tiers[2]?.minimum ?? 30000) - 1))
...
const tierMismatch = computed(() => {
  const factions = rankedConfig.value?.factions ?? []
...
function updateTier(index: number, key: SharedTierKey, event: Event) {
  ...
  for (const faction of rankedConfig.value?.factions ?? []) if (faction.tiers[index]) faction.tiers[index][key] = value
}
function normalizeTiers(sourceIndex: number) {
  ...
  for (const faction of config.factions) faction.tiers.forEach((tier, index) => {
...
async function load() {
  ...
  try {
    ...
    catalog.value = cards
    rankedConfig.value = normalizeRankedConfig(ranked)
...
function validateRankedTimeControl(config: RankedConfig) {
  ...
  const fields = Object.values(timing)
  if (fields.some(value => !Number.isSafeInteger(value))) return '排位时限必须填写整数秒数'
  if (timing.totalTimeSeconds < 300 || timing.totalTimeSeconds > 7200) return '总操作时限需为 300–7200 秒'
  if (timing.operationTimeSeconds < 15 || timing.operationTimeSeconds > 900 || timing.operationTimeSeconds > timing.totalTimeSeconds)
    return '单次操作时限需为 15–900 秒，且不得超过总操作时限'
  if (timing.reconnectGraceSeconds < 15 || timing.reconnectGraceSeconds > 900) return '断线重连宽限需为 15–900 秒'
  if ([timing.disasterDecisionSeconds, timing.mulliganDecisionSeconds].some(value => value < 10 || value > 300))
    ...
    <section v-if="activeSection !== 'versions'" class="panel config-panel">
      ...
      <div class="config-grid section-grid">
        <template v-if="activeSection === 'ranked' && rankedConfig">
          <fieldset><legend>定级与广播</legend><label>定级场次<input v-model.number="rankedConfig.placementMatches" type="number" min="1" max="20"/></label><label>定级七曜上限<input v-model.number="rankedConfig.placementMaximum" type="number" min="0" :max="placementMaximumLimit"/><small>须低于第三段门槛，当前最多 {{ placementMaximumLimit }}</small></label><label class="toggle-row wide"><span><b>启用排位快讯</b><small>总开关关闭后不再生成新的排位广播。</small></span><input v-model="rankedConfig.broadcastEnabled" type="checkbox"/></label></fieldset>
          <fieldset class="wide ranked-broadcast-config" data-ui-contract="ranked-broadcast-config"><legend>排位广播规则</legend><p class="wide field-help">设置只影响下一条广播；当前正在播放的广播不会被中断，回放不会重复广播。</p><label>单条显示时长（秒）<input v-model.number="rankedConfig.broadcast.displaySeconds" type="number" min="5" max="120"/></label><label>进入大厅后延迟（秒）<input v-model.number="rankedConfig.broadcast.lobbyDelaySeconds" type="number" min="0" max="120"/></label><label>两条广播间隔（秒）<input v-model.number="rankedConfig.broadcast.intervalSeconds" type="number" min="3" max="600"/></label><label>连胜广播门槛<input v-model.number="rankedConfig.broadcast.winStreakThreshold" type="number" min="2" max="100"/></label><label>终结连胜门槛<input v-model.number="rankedConfig.broadcast.streakEndedThreshold" type="number" min="2" max="100"/></label><label>最低段位<select v-model.number="rankedConfig.broadcast.minimumTierIndex"><option v-for="(tier,index) in rankedConfig.factions[0]?.tiers" :key="index" :value="index">第 {{ index + 1 }} 段 · {{ tier.name }}</option></select></label><label class="toggle-row"><span><b>连胜</b></span><input v-model="rankedConfig.broadcast.winStreakEnabled" type="checkbox"/></label><label class="toggle-row"><span><b>终结连胜</b></span><input v-model="rankedConfig.broadcast.streakEndedEnabled" type="checkbox"/></label><label class="toggle-row"><span><b>最高段位晋升</b></span><input v-model="rankedConfig.broadcast.highestTierEnabled" type="checkbox"/></label><label class="toggle-row"><span><b>派名称号</b></span><input v-model="rankedConfig.broadcast.factionTitleEnabled" type="checkbox"/></label><label class="toggle-row"><span><b>最强主宰称号</b></span><input v-model="rankedConfig.broadcast.masterTitleEnabled" type="checkbox"/></label></fieldset>
          <fieldset class="wide ranked-time-control" data-ui-contract="ranked-time-control-config"><legend>排位用时</legend><p class="wide field-help">单位均为秒。保存后只影响新创建的排位对局；进行中对局继续使用创建时冻结的时限，重启恢复也不会被新配置覆盖。</p><label>总操作时限<input v-model.number="rankedConfig.timeControl.totalTimeSeconds" type="number" min="300" max="7200" step="1"/><small>300–7200 秒；双方分别计算。</small></label><label>单次操作时限<input v-model.number="rankedConfig.timeControl.operationTimeSeconds" type="number" min="15" max="900" step="1"/><small>15–900 秒，且不得超过总时限。</small></label><label>断线重连宽限<input v-model.number="rankedConfig.timeControl.reconnectGraceSeconds" type="number" min="15" max="900" step="1"/><small>15–900 秒。</small></label><label>天灾禁选／选择<input v-model.number="rankedConfig.timeControl.disasterDecisionSeconds" type="number" min="10" max="300" step="1"/><small>每一步 10–300 秒；超时由服务器从合法候选中自动选择。</small></label><label>手牌调度<input v-model.number="rankedConfig.timeControl.mulliganDecisionSeconds" type="number" min="10" max="300" step="1"/><small>10–300 秒；超时保留原手牌。</small></label></fieldset>
          ...
          <fieldset class="wide"><legend>五段位共同规则</legend><p class="wide field-help">同等级共用门槛与结算数值，仅段位名称分派系填写。只有第五段位玩家可获得派系排名称号。</p><p v-if="rankedConfig.pendingGradient" class="wide locked-note">下赛季梯度已排定；为避免误改本赛季结算，本赛季数值暂时锁定。</p>
            <p v-if="tierMismatch" class="wide notice" role="alert">历史配置存在不同派系的段位数值差异。请确认以哪个派系为准后统一，系统不会静默覆盖。</p>
            <div v-if="tierMismatch" class="wide"><button v-for="(faction, index) in rankedConfig.factions" :key="faction.id" type="button" @click="normalizeTiers(index)">以{{ faction.name }}的数值统一</button></div>
            <div class="wide ranked-shared-tiers"><article v-for="(tier, index) in rankedConfig.factions[0]?.tiers" :key="index">
              <h3>第 {{ index + 1 }} 段位</h3>
              <div class="tier-name-fields"><label v-for="faction in rankedConfig.factions" :key="faction.id">{{ faction.name }}名称<input v-if="faction.tiers[index]" v-model="faction.tiers[index].name"/></label></div>
              <div class="tier-number-fields"><label v-for="field in sharedTierFields" :key="field.key">{{ field.label }}<input :value="tier[field.key]" type="number" min="0" step="1" :disabled="tierMismatch || Boolean(rankedConfig.pendingGradient)" @input="updateTier(index, field.key, $event)"/></label></div>
            </article></div>
          </fieldset>
          <fieldset v-if="rankedConfig.pendingGradient" class="wide" data-ui-contract="pending-ranked-gradient"><legend>下赛季待生效梯度</legend><p class="wide field-help">当前赛季继续使用上方数值。切换离开赛季 {{ rankedConfig.pendingGradient.afterSeasonId }} 时，以下梯度会一次性生效；隐藏匹配分不会重置。</p><div class="wide ranked-shared-tiers"><article v-for="(tier,index) in rankedConfig.pendingGradient.tiers" :key="tier.name"><h3>第 {{ index + 1 }} 段位 · {{ tier.name }}</h3><div class="tier-number-fields"><span>门槛 <b>{{ tier.minimum.toLocaleString() }}</b></span><span>基础胜负 <b>±{{ tier.baseDelta.toLocaleString() }}</b></span><span>连胜上限 <b>+{{ tier.winStreakCap.toLocaleString() }}</b></span><span>连败保护 <b>{{ tier.lossProtectionCap.toLocaleString() }}</b></span><span>差值修正 <b>±{{ tier.ratingGapCap.toLocaleString() }}</b></span><span>终结连胜 <b>+{{ tier.streakTerminationReward.toLocaleString() }}</b></span></div></article></div></fieldset>
          <fieldset class="wide"><legend>主宰最强玩家称号</legend><p class="wide field-help">按本赛季使用该主宰参加过排位的已定级玩家比较七曜值；同分时依次比较该主宰胜场、场次及隐藏匹配分。</p><div class="wide ranked-master-title-grid"><PagedCollection :items="rankedConfig.masterTitles" v-slot="{ items: paged19657 }"><label v-for="master in paged19657" :key="master.masterId"><span>{{ master.masterName }}<small>{{ master.masterId }}</small></span><input v-model="master.title" :aria-label="`${master.masterName}最强玩家称号`"/></label></PagedCollection></div></fieldset>
          <fieldset class="wide"><legend>近期排位快讯</legend><PagedCollection :items="rankedBroadcasts" v-slot="{ items: paged20048 }"><article v-for="item in paged20048" :key="item.id" class="broadcast-row"><span>{{ item.message }}<small>{{ new Date(item.createdAt).toLocaleString() }}</small></span><button @click="deleteBroadcast(item.id)">删除</button></article></PagedCollection><span v-if="!rankedBroadcasts.length">暂无排位快讯</span></fieldset>
        </template>

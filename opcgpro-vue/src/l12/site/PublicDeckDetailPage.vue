...
function scrollToSection(section: typeof sectionTabs[number]['id']) {
...
<template>
  <main class="public-deck-detail" data-ui-contract="public-deck-detail-page" :aria-busy="actionBusy">
    ...
    <template v-else>
      ...
      <section id="public-deck-guide" class="content-panel detail-anchor-section" data-detail-section="guide">
        ...
        <div v-if="Object.values(details.guide).some(Boolean)" class="reading-sections">
      ...
      <section id="public-deck-matchups" class="content-panel detail-anchor-section" data-detail-section="matchups">
        <header><div><h2>对局建议</h2><p>按敌方主宰查看作者提供的思路、关键牌与换牌建议。</p></div></header>
        <div v-if="details.matchups.length" class="matchup-list"><article v-for="row in details.matchups" :key="row.opponentMasterId"><h3>对阵 {{ masterName(row.opponentMasterId) }}</h3><p><b>思路</b>{{ row.notes || '作者暂未填写。' }}</p><p><b>关键牌</b>{{ row.keyCards || '作者暂未填写。' }}</p><p><b>换牌</b>{{ row.suggestedSwaps || '作者暂未填写。' }}</p></article></div>

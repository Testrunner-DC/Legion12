...
<template>
  <div class="deck-page">
    ...
    <template v-if="tab === 'mine'">
      ...
      <section v-if="mine.length" class="mine-grid"><article v-for="deck in mine" :key="deck.name"><div class="deck-banner"><CardImage :card-id="deck.masterId" :legacy-url="byId.get(deck.masterId)?.imageUrl" :alt="byId.get(deck.masterId)?.nameZh || '主宰'" intent="thumb" fit="cover" object-position="center 22%"/><span>{{ byId.get(deck.masterId)?.nameZh || '主宰' }}</span></div><h2>{{ deck.name }}</h2><p>{{ deckCountSummary(deck.cardIds, byId).label }} 张主牌 · {{ deck.moraleIds.length }} 张士气</p><div><router-link :to="editorLink(deck.name)">编辑</router-link><button @click="copyCode(deck)">复制牌库码</button><button @click="previewImage(deck)">生成牌库图</button></div></article></section>

...
<template>
  <section class="alternate-admin">
    ...
    <section class="art-list"><h4>已登记异画</h4><article v-for="art in arts" :key="art.id"><CardImage v-if="art.builtIn" :card-id="art.cardImageId" :alt="art.displayName" intent="thumb"/><img v-else :src="art.thumbnailUrl" :alt="art.displayName"><div><b>{{ art.displayName }}</b><span>{{ art.artCode || '待补异画编号' }} · {{ art.baseCardId }} · {{ cardById.get(art.baseCardId)?.nameZh || '原卡资料加载中' }}</span><small>{{ art.productName || '未归属产品' }} · {{ art.builtIn ? '内置异画，已在画廊展示，可授予' : art.active ? '已在画廊展示，可授予' : '已停用，不展示不再授予' }}</small></div><button v-if="!art.builtIn" @click="editArt(art)">编辑</button></article><p v-if="!arts.length">尚未登记异画。</p></section>

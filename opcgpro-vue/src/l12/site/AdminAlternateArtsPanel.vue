...
async function previewBetaGrant() {
  ...
  try { betaGrantPreview.value = await adminApi.previewAlternateArtRankedParticipants(betaGrantForm) }
...
async function dispatchBetaGrant() {
  ...
  try {
    const saved = await adminApi.dispatchAlternateArtRankedParticipants(betaGrantForm)
    grants.value = [...saved, ...grants.value.filter(item => !saved.some(row => row.id === item.id))]
    notice(`已完成内测排位参与者派发：${betaGrantPreview.value.toGrant} 名新增权益`)

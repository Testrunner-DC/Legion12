...
const requestedTab = computed(() => typeof route.meta.adminSection === 'string' ? route.meta.adminSection
...
const adminMatchId = ref(typeof route.params.matchId === 'string' ? route.params.matchId : '')
...
const bugSearch = ref(typeof route.params.bugId === 'string' ? route.params.bugId : '')
...
async function updateBug(item: BugReport) {
  ...
  try { const updated = await adminApi.updateBug(item.id, { status: item.status, priority: item.priority, assignee: item.assignee, adminNotes: item.adminNotes, comment: bugComments[item.id], fixCommit: item.fixCommit, regressionTest: item.regressionTest, deployedVersion: item.deployedVersion, verifiedBy: item.verifiedBy, verifiedAt: item.verifiedAt, duplicateOf: item.duplicateOf }); bugs.value = bugs.value.map(bug => bug.id === updated.id ? updated : bug); bugComments[item.id] = ''; notice.value = `${item.id} 已更新并写入审计记录` } catch (error) { notice.value = error instanceof Error ? error.message : '更新失败' }
}
function bugActionLabel(action: string) { return ({ created: '建立反馈', status: '状态变更', priority: '优先级变更', assignee: '负责人变更', notes: '处理摘要变更', comment: '追加处理记录', 'fix-commit': '关联修复提交', 'regression-test': '关联回归测试', 'deployed-version': '记录部署版本', 'verified-by': '记录验证人', 'verified-at': '记录验证时间', 'duplicate-of': '关联重复问题' } as Record<string, string>)[action] || action }
function switchAdminTab(next: AdminTab) {
  const destination = availableAdminTabs.value.find(item => item.id === next)
  if (!destination) return
  void router.push(destination.path)
}
function openAdminMatch(matchId: string) {
  if (!hasPermission('admin.matches.read')) return
  void router.push({ path: `/admin/matches/archive/${encodeURIComponent(matchId)}` })
}
...
async function loadEffects(resetPage = false) {
  ...
  try {
    ...
    if (requestedCardId && selectedEffect.value?.cardId !== requestedCardId) {
      const requested = page.items.find(card => card.cardId === requestedCardId)
      if (requested) await selectEffect(requested)
    }
...
async function loadCurrentAdminSection() {
  ...
  pendingAdminSection = pendingAdminSection.catch(() => {}).then(async () => {
    ...
    else if (current === 'effects') await loadEffects(true)
...
watch(() => [route.meta.adminSection, route.params.matchId, route.params.bugId, route.params.cardId, authState.verified, platformState.account?.id, platformState.account?.permissionVersion], () => {
  adminGeneration++
  adminMatchId.value = typeof route.params.matchId === 'string' ? route.params.matchId : ''
  if (typeof route.params.bugId === 'string') bugSearch.value = route.params.bugId
  if (typeof route.params.cardId === 'string') effectSearch.value = route.params.cardId
  loadedTabs.delete(tab.value as AdminTab)
...
<template>
  <div class="admin-module-page">
    ...
    <AdminRiskActionDialog v-if="temporaryPassword" title="一次性临时密码已生成" :target="temporaryPasswordAccount" target-label="账号" impact="该密码只显示这一次。请通过受控渠道交给账号本人；关闭后后台无法再次查看。" confirm-label="我已安全保存" severity="warning" :allow-cancel="false" :busy="false" @confirm="clearTemporaryPassword">
      ...
      <header v-else class="section-heading"><span>管理后台 / {{ selectedSection.domain }}</span><h2>{{ selectedSection.label }}</h2></header>
      ...
      <section v-else-if="tab === 'accounts'" class="panel account-panel">
        ...
        <PagedCollection :items="activeAccounts" v-slot="{ items: paged30632 }"><div v-for="account in paged30632" :key="account.id" class="account-row">
          <b><router-link :to="`/admin/users/accounts/${encodeURIComponent(account.id)}`">{{ account.username }}</router-link><small :data-disabled="account.disabled">{{ account.disabled ? '已禁用' : '正常' }}<template v-if="account.mustChangeUsername"> · 待修改用户名</template><template v-if="account.mustChangePassword"> · 必须修改密码</template><template v-if="account.emailVerified"> · 邮箱 {{ account.emailMasked }}</template><template v-if="account.disabledReason"> · {{ account.disabledReason }}</template></small></b>
...
.admin-module-page{min-width:0;font-family:'Microsoft YaHei','微软雅黑',sans-serif}.admin-module-page small{color:#d5b85e;font:900 14px monospace;letter-spacing:.16em}.admin-module-page p{color:#7d898e;font-size:14px;line-height:1.7}.admin-content{min-width:0}.overview-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.overview-grid>header{grid-column:1/-1}.overview-card{display:flex;min-height:130px;flex-direction:column;align-items:flex-start;justify-content:flex-end;gap:7px;padding:17px;border:1px solid #35424a;background:#101821;color:#fff;text-align:left;text-decoration:none}.overview-card:hover{border-color:#c1a44e;background:#171b1d}.overview-card b{font-size:22px}.overview-card span{color:#87949a;font-size:14px}.panel,.denied{border:1px solid #35424a;background:#101821;padding:20px}.panel>header{display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #36434a;padding-bottom:13px}.panel h2{margin:0}.panel button,.panel select,.panel input,.panel textarea{border:1px solid #4c5961;background:#080e13;color:#fff;font:700 14px 'Microsoft YaHei';padding:9px}.bug-row{display:grid;grid-template-columns:1fr 300px;gap:18px;padding:18px 0;border-bottom:1px solid #303c43}.bug-id{text-decoration:none}.bug-summary code{color:#dfc36f}.bug-summary b,.bug-summary span,.bug-summary small{display:block}.bug-summary b{margin:7px 0;font-size:16px}.bug-summary span,.bug-summary small{color:#718087;font-size:14px}.bug-summary p{color:#c7ccca;white-space:pre-wrap;overflow-wrap:anywhere}.bug-evidence{display:grid;grid-template-columns:100px minmax(0,1fr);gap:5px 10px;margin:12px 0;padding:12px;border:1px solid #2f3c43;background:#0a1117}.bug-evidence dt{color:#7f8d92}.bug-evidence dd{margin:0;overflow-wrap:anywhere}.bug-admin{display:grid;grid-template-columns:1fr 1fr;gap:7px}.bug-admin input,.bug-admin textarea,.bug-admin button{grid-column:1/-1}.account-row{display:grid;grid-template-columns:1fr 1.5fr 180px 90px minmax(320px,1fr);align-items:center;gap:10px;padding:12px;border-bottom:1px solid #303c43}.account-row.head{color:#7e8a90;font-size:14px}.account-row a{color:#e2c56f}.content-editor label{display:block;margin-top:16px;color:#b8c0c1;font-size:14px;font-weight:900}.content-editor input,.content-editor textarea{box-sizing:border-box;width:100%;margin-top:7px}.denied{display:flex;flex-direction:column;gap:7px;margin-top:22px}.denied span,.empty{color:#7e8a90}.notice{position:sticky;z-index:20;bottom:12px;margin-top:12px;padding:10px;border-left:3px solid #d1b25c;background:#241c0a;color:#edd584!important}
...
@media(max-width:850px){
  .admin-module-page{overflow-x:clip}

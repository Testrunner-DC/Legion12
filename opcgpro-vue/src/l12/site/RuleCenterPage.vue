...
const ruleCenter = ref<RuleCenterDocument>({ coreBlocks: [], quickStart: [], terms: [], tournament: [], versions: [] })
...
const categoryDefinitions = [
  { id: 'battle', label: '进攻与战斗', keywords: ['进攻', '兵力', '伤害', '击杀', '支援', '天灾'] },
  { id: 'effects', label: '效果与响应', keywords: ['效果', '响应', '堆叠', '费用', '展示', '隐藏信息'] },
  { id: 'zones', label: '区域与状态', keywords: ['登场', '离场', '墓地', '手牌', '控制者', '所有者'] },
  { id: 'card', label: '单卡裁定', keywords: [] },
] as const
const popularKeywords = ['登场', '响应', '费用', '进攻', '兵力', '天灾']
const topics = computed(() => [...new Set(ruleCenter.value.coreBlocks.map(block => block.topic).filter(Boolean))] as string[])
const allRulings = computed(() => mergedRulings(dynamicRulings.value).filter(item => item.status === 'published'))
function search(value: string, queryText: string) { return !queryText || value.toLocaleLowerCase().includes(queryText.toLocaleLowerCase()) }
function rulingText(item: RuleRuling) { return [item.question, item.answer, item.category, item.tags.join(' '), item.cardIds.join(' '), item.productIds.join(' ')].join(' ') }
function rulingCategory(item: RuleRuling) {
  if (item.scope === 'card' || item.scope === 'errata') return 'card'
  return categoryDefinitions.find(category => category.keywords.some(keyword => rulingText(item).includes(keyword)))?.id ?? 'effects'
}
function displayRuleText(text: string) {
  return text.split('\n').map(line => line.replace(/（?\s*P\.[^\s\n）]*\s*）?/gi, '').replace(/[.。]{2,}\s*\d+\s*$/, '').trimEnd()).filter(line => line.trim()).join('\n')
}
const ruleResults = computed(() => ruleCenter.value.coreBlocks.filter(block => block.topic !== '目录' && (topic.value === 'all' || block.topic === topic.value) && search([block.topic, block.chapter, displayRuleText(block.text)].filter(Boolean).join(' '), query.value.trim())))
const filteredEntries = computed(() => {
  const entries: RuleCenterEntry[] = tab.value === 'quick-start' ? ruleCenter.value.quickStart : ruleCenter.value.terms
  return entries.filter(item => search([item.title, item.body, item.tags.join(' ')].join(' '), query.value.trim()))
})
const faqResults = computed(() => allRulings.value.filter(item => {
  ...
  return matchesMode && (faqCategory.value === 'all' || rulingCategory(item) === faqCategory.value) && search(rulingText(item), query.value.trim())
}))
...
<template>
  <div class="rules-page">
    <header class="rules-head"><div><small>RULES CENTER</small><h1>规则中心</h1><p>以章节、稳定条目和版本记录组织电子资料；不显示页码。公开 FAQ 只收录已确认的现行裁定。</p></div><button v-if="tab === 'core'" @click="printRules">打印 / 保存 PDF</button></header>
    ...
    <template v-else-if="tab === 'quick-start' || tab === 'terms'">
      ...
      <section class="entry-grid"><article v-for="item in filteredEntries" :key="item.id" :data-status="item.status"><small>{{ item.sourceRef }}</small><h3>{{ item.title }}</h3><p>{{ item.body }}</p><div><span v-for="tag in item.tags" :key="tag">{{ tag }}</span></div></article><div v-if="!filteredEntries.length" class="empty">没有匹配的内容</div></section>
    </template>
    ...
    <template v-else-if="tab === 'construction'">
      <section class="section-lead"><small>DECK CONSTRUCTION</small><h2>构筑与限制</h2><p>此页读取当前公开运营规则。卡牌限制会随已发布版本更新，不以页面缓存或其他游戏的限制替代。</p></section>
    ...
    <template v-else-if="tab === 'tournament'">
      <section class="section-lead"><small>TOURNAMENT RULES</small><h2>赛事规则</h2><p>赛事专用赛制、时限、裁判与申诉规则只在正式发布后生效；未发布的项目不会以其他游戏惯例补足。</p></section><section class="entry-grid"><article v-for="item in ruleCenter.tournament" :key="item.id"><small>已发布 · {{ item.sourceRef }}</small><h3>{{ item.title }}</h3><p>{{ item.body }}</p></article><div v-if="!ruleCenter.tournament.length" class="empty">赛事规则尚未由管理员审核发布</div></section>
    </template>
    <template v-else>
      <section class="section-lead"><small>VERSION HISTORY</small><h2>版本记录</h2><p>电子资料以版本、记录日期和稳定条目追溯；未提供日期的资料会明确标记，不以页码补代。</p></section><section class="version-list"><article v-for="item in ruleCenter.versions" :key="item.id"><div><small>{{ item.kind }} · 已发布</small><h3>{{ item.title }}<span v-if="item.version">{{ item.version }}</span></h3><p>{{ item.summary }}</p></div><aside><b>{{ item.sourceRef }}</b><span v-if="item.recordedAt">记录：{{ item.recordedAt }}</span><span v-if="item.effectiveAt">生效：{{ item.effectiveAt }}</span></aside></article><div v-if="!ruleCenter.versions.length" class="empty">版本记录尚未由管理员审核发布</div></section>
    </template>
  </div>
</template>
...
@media(max-width:520px){.rules-page{padding:0 12px 48px}.rule-tabs button{padding:12px}.faq-mode-tabs button{padding:13px 12px}.faq-category-grid{grid-template-columns:1fr}.faq-question{grid-template-columns:48px minmax(0,1fr) 28px;padding:14px 10px;gap:8px}.faq-answer{grid-template-columns:48px minmax(0,1fr);padding:15px 10px;gap:8px}}

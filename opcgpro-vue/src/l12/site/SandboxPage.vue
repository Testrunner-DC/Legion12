...
import { ensureOfficialPrebuiltDecks, loadDeckCatalog, type DeckCard, type SavedL12Deck } from '@/l12/decks'
  ...
  <div class="sandbox-page">
    <section>
      ...
      <div class="sandbox-grid">
        ...
        <label><b>对手牌库</b><DeckProfile v-if="opponentDeck" compact :master-id="opponentDeck.masterId" :master-name="byId.get(opponentDeck.masterId)?.nameZh" :name="opponentDeck.name" context="对手"/><select v-model="opponentDeckName"><option v-for="deck in decks" :key="`enemy-${deck.name}`" :value="deck.name">{{ deck.name }}</option></select></label>

...
<template>
  ...
  <Teleport to="body">
    <section v-if="modelValue" class="mobile-filter-mask" role="dialog" aria-modal="true" :aria-label="title" @click.self="close">
...
.mobile-filter-mask{position:fixed;z-index:5000;inset:0;display:grid;align-items:end;background:#020609b8;backdrop-filter:blur(7px)}.mobile-filter-sheet{display:grid;width:100%;max-height:min(82dvh,720px);grid-template-rows:auto minmax(0,1fr) auto;border:1px solid #59666b;border-bottom:0;background:#10191f;color:#f1f0e8;box-shadow:0 -22px 68px #000}.mobile-filter-sheet header{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:15px 18px;border-bottom:1px solid #36434a}.mobile-filter-sheet small{color:#e1bd5d;font-size:11px;font-weight:900;letter-spacing:.13em}.mobile-filter-sheet h2{margin:3px 0 0;font-size:18px}.mobile-filter-sheet header button{display:grid;width:38px;height:38px;place-items:center;border:1px solid #627078;background:#080e12;color:#fff;font-size:24px}.mobile-filter-content{min-height:0;overflow:auto;padding:16px 18px calc(16px + env(safe-area-inset-bottom));overscroll-behavior:contain}.mobile-filter-sheet footer{display:grid;grid-template-columns:1fr 1.4fr;gap:9px;padding:12px 18px max(12px,env(safe-area-inset-bottom));border-top:1px solid #36434a;background:#0c1318}.mobile-filter-sheet footer button{min-height:42px;border:1px solid #5b6970;background:#172129;color:#f1f1eb;font-size:14px;font-weight:900}.mobile-filter-sheet footer .primary{border-color:#e0bb5b;background:#e0bb5b;color:#0c1012}

...
<template>
  <div class="tournament-page" :class="{ embedded: props.embedded }">
    <header class="page-head"><div><small>TOURNAMENT OPERATIONS</small><h1>{{ props.adminMode ? '赛事管理' : '赛事中心' }}</h1><p>{{ props.adminMode ? '查看并管理玩家创建的全部赛事；全局管理操作保留版本校验、理由与审计记录。' : '服务端保存报名、牌库可见性快照、配对、判罚与审计记录；主办者与裁判权限仅在当前赛事内生效。' }}</p></div><button v-if="!props.adminMode && hasPermission('tournaments.create')" class="gold" @click="tab = 'create'">举办赛事</button></header>
    ...
    <div v-if="detail" class="mask" @click.self="detailId=null"><section class="detail">
      ...
      </template><footer class="detail-actions"><template v-if="!props.adminMode&&detail.status==='registration'&&!isParticipant(detail)&&deckDrafts[detail.id]"><select v-model="deckDrafts[detail.id].name"><option disabled value="">选择账号牌库</option><option v-for="deck in savedDecks" :key="deck.name" :value="deck.name">{{ deck.name }}</option></select><button class="gold" :disabled="busy||!deckDrafts[detail.id].name" @click="join(detail)">选择牌库并报名</button></template><button @click="copyShareLink(detail)">复制稳定分享链接</button><button v-if="!props.adminMode&&detail.status!=='completed'&&isParticipant(detail)&&!isOrganizer(detail)" class="danger" :disabled="busy" @click="dropRegistration(detail)">{{ detail.status==='running'?'退赛':'退出赛事' }}</button><button v-if="(isOrganizer(detail)||canGloballyManage)&&detail.status==='registration'" class="gold" :disabled="busy||detail.participants.filter(person=>!person.dropped).length<2" @click="startTournament(detail)">正式开启赛事</button><button v-if="(isOrganizer(detail)||canGloballyManage)&&detail.status==='running'" class="danger" :disabled="busy" @click="finishTournament(detail)">结束并归档赛事</button></footer>
...
@media(max-width:520px){.tournament-page{padding:14px 10px 34px}.page-head{gap:8px;padding-bottom:12px}.page-head small,.create-panel small,.registry small,.detail small{font-size:10px;letter-spacing:.13em}.page-head h1{margin:3px 0;font-size:24px}.page-head p,.create-panel p,.migration p,.pending p,.registry p,.migration-preview,.registry dt,.registry dd,.form-grid label,.form-grid fieldset span,.summary span,.chips span,.detail>p,.rules,.participants span,.participants em,.standings>div,.bracket article,.matches article>header span,.matches p,.matches li{font-size:12px}.page-head button,.create-action{min-height:38px;padding:8px 11px;font-size:12px}.migration{gap:7px;margin-top:10px;padding:10px}.migration button{min-height:34px;padding:6px 8px;font-size:12px}.tabs{margin-top:10px}.tabs button{min-height:38px;padding:8px;font-size:12px}.registry,.create-panel{margin-top:10px}.registry>input{min-height:38px;width:calc(100% - 20px);margin:10px;padding:8px;font-size:12px}.registry>article{gap:10px;padding:11px}.registry h2{margin:2px 0;font-size:16px}.registry dl{gap:5px}.actions{flex-wrap:wrap}.actions select,.participants select{min-height:34px;padding:6px;font-size:12px}.actions button,.detail button,.round-controls button,.staff-editor button,.pending button,.detail-actions select{min-height:34px;padding:6px 8px;font-size:12px}.create-panel{padding:14px}.create-panel>header{padding-bottom:10px}.form-grid{gap:9px;margin-top:12px}.form-grid input,.form-grid select,.form-grid textarea,.participants input,.matches textarea,.round-controls input,.staff-editor input,.pending input,.reference input{min-height:38px;margin-top:5px;padding:8px;font-size:12px}.form-grid fieldset{gap:5px;padding:5px}.form-grid fieldset button{min-height:32px;padding:6px;font-size:12px}.create-action{min-height:40px;margin-top:13px}.mask{padding:8px}.detail{width:calc(100vw - 16px);max-height:calc(100vh - 16px)}.detail>header{padding:11px}.detail h2{font-size:18px}.summary,.chips{gap:5px;padding:9px 11px}.summary span,.chips span{padding:4px 6px}.detail>p,.detail>h3,.rules,.participants,.round-controls,.matches,.staff-editor,.pending,.standing-snapshot,.bracket{margin-right:11px;margin-left:11px}.detail>p{margin-top:11px;margin-bottom:11px}.detail>h3{margin-top:15px;font-size:15px}.rules{gap:7px;padding:8px}.staff-editor{gap:5px;padding:8px}.staff-editor input{min-height:34px}.pending{margin-top:11px;padding:9px}.participants{gap:5px}.participants>div{gap:6px;padding:7px}.standing-snapshot summary{padding:7px;font-size:12px}.standings>div{gap:5px;padding:7px}.bracket{gap:7px}.bracket>section{min-width:190px;padding:8px}.bracket article{gap:4px;margin-top:6px;padding:6px}.round-controls{gap:5px;margin-bottom:7px}.matches{gap:6px}.matches>article{padding:8px}.matches article>header{gap:5px}.matches article>div{gap:5px;margin-top:7px}.matches textarea{min-height:46px}.matches footer{gap:4px;margin-top:6px}.detail-actions{gap:6px;margin-top:14px;padding:10px 11px}.toast{right:10px;bottom:10px;max-width:calc(100vw - 20px);padding:8px 10px;font-size:12px}}
</style>

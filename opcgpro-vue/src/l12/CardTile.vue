...
function useAttachedCardBack(event: Event) {
  const image = event.currentTarget as HTMLImageElement
  if (image.dataset.fallbackApplied === 'true') return
  image.dataset.fallbackApplied = 'true'
  image.src = CARD_IMAGE_PLACEHOLDER
}
  ...
  <button class="card-tile" :data-card-instance-id="card.instanceId" :class="[{ selected, tapped: card.tapped, compact, 'horizontal-card': isHorizontalCardType(card.cardType), 'has-status-effects': statusIndicators.length }, `type-${card.cardType}`]" @click="$emit('select')">
    ...
    <span v-if="showFace && attachedGroups.length" class="attached-card-orbs" aria-label="叠放卡牌">
      <span v-for="group in attachedGroups" :key="group.card.cardId" class="attached-card-orb" role="button" tabindex="0"
        ...
        :aria-label="`${group.card.name}${group.count > 1 ? ` ×${group.count}` : ''}`"
        ...
        <img :src="roundCardUrl(group.card.cardId, group.card.imageUrl) || CARD_IMAGE_PLACEHOLDER" alt="" @error="useAttachedCardBack" />
...
<style scoped>
.card-tile{container-type:inline-size;--l12-card-edge:clamp(1px,2.4cqw,4px);--l12-card-stat-font:clamp(6px,11cqw,18px);--l12-card-stat-line:clamp(8px,16cqw,24px);--l12-card-stat-pad-x:clamp(1px,3cqw,5px);--l12-card-stat-pad-y:clamp(0px,1.2cqw,2px);--l12-card-status-edge:3px;--l12-card-status-top:50px;--l12-card-status-size:19.5px;--l12-card-status-gap:2px;--l12-card-status-font:max(12px,var(--l12-board-micro,9px));--l12-card-keyword-bottom:40px;--l12-card-keyword-gap:4px;--l12-card-keyword-pad:4px;--l12-card-keyword-font:clamp(10px,var(--l12-board-micro,9px),11.25px);--l12-card-attached-size:22px;--l12-card-attached-bottom:27px}
.card-cost,.card-power,.card-disaster{padding:var(--l12-card-stat-pad-y) var(--l12-card-stat-pad-x)!important;font-size:var(--l12-card-stat-font)!important;line-height:var(--l12-card-stat-line)!important}
.card-cost{left:var(--l12-card-edge)!important;top:var(--l12-card-edge)!important}.card-disaster{right:var(--l12-card-edge)!important;top:var(--l12-card-edge)!important}.card-power{bottom:var(--l12-card-edge)!important}
...
.attached-card-orbs{position:absolute;z-index:9;right:var(--l12-card-status-edge);bottom:var(--l12-card-attached-bottom);display:flex;max-width:calc(100% - var(--l12-card-status-edge) - var(--l12-card-status-edge));flex-direction:row-reverse;align-items:center;gap:var(--l12-card-status-gap);pointer-events:auto}.attached-card-orb{position:relative;display:grid;width:var(--l12-card-attached-size);height:var(--l12-card-attached-size);min-width:var(--l12-card-attached-size);place-items:center;padding:0;overflow:visible;border:1px solid #d8d2bd;border-radius:50%;background:#090b0c;box-shadow:0 2px 7px #000;cursor:pointer}.attached-card-orb img{position:static!important;inset:auto!important;width:100%!important;height:100%!important;border-radius:50%;background:#090b0c;object-fit:cover;object-position:center 14%;transform:scale(1.08)}.attached-card-orb b{position:absolute;z-index:2;left:50%;top:calc(0px - var(--l12-card-status-gap) - var(--l12-card-status-gap) - var(--l12-card-status-gap));display:grid;min-width:var(--l12-card-status-size);height:var(--l12-card-status-size);place-items:center;padding:0 var(--l12-card-status-gap);border:1px solid #f0e7c8;border-radius:50%;background:#111;color:#fff;font-size:var(--l12-card-status-font);line-height:1;transform:translateX(-50%)}.attached-card-orb:hover,.attached-card-orb:focus-visible{border-color:#72d9df;box-shadow:0 0 9px rgba(112,217,223,.8);outline:none}
.attached-card-orb{border-color:#f0f2ef;box-shadow:0 0 0 2px rgba(194,199,196,.72),0 2px 7px #000}
.card-status-icons{left:var(--l12-card-status-edge);right:auto;top:var(--l12-card-status-top);width:calc(100% - var(--l12-card-status-edge) - var(--l12-card-status-edge));height:auto;max-height:none;flex-flow:row wrap;align-content:flex-start;align-items:flex-start;gap:var(--l12-card-status-gap)}.card-status-icon{box-sizing:border-box;width:var(--l12-card-status-size);min-width:var(--l12-card-status-size);height:var(--l12-card-status-size);flex:0 0 var(--l12-card-status-size);font-size:var(--l12-card-status-font)}
.card-keyword-stack,.card-tile.has-status-effects .card-keyword-stack{left:50%;top:auto;bottom:var(--l12-card-keyword-bottom);display:flex;width:calc(100% - var(--l12-card-status-edge) - var(--l12-card-status-edge));max-width:calc(100% - var(--l12-card-status-edge) - var(--l12-card-status-edge));flex-direction:column-reverse;align-items:stretch;gap:var(--l12-card-keyword-gap);transform:translateX(-50%)}.card-keyword-row{display:flex;width:100%;min-width:0;align-items:center;justify-content:center;gap:var(--l12-card-status-gap)}.card-keyword-row>.card-keyword{box-sizing:border-box;min-width:0;max-width:calc((100% - var(--l12-card-status-gap) - var(--l12-card-status-gap))/3);padding:var(--l12-card-keyword-pad);font-size:var(--l12-card-keyword-font);text-align:center}
.card-tile.compact{--l12-card-status-top:34px;--l12-card-status-size:17px;--l12-card-keyword-bottom:34px;--l12-card-keyword-gap:2px;--l12-card-keyword-pad:2px;--l12-card-keyword-font:var(--l12-board-micro,9px)}.card-tile.compact .card-status-icons{height:auto}.card-tile.compact .card-keyword-stack,.card-tile.compact.has-status-effects .card-keyword-stack{top:auto}.card-tile.compact .card-keyword-row>.card-keyword{max-width:calc((100% - var(--l12-card-status-gap) - var(--l12-card-status-gap))/3)}
</style>

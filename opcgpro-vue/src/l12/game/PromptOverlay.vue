...
const setupClockPlayerIndex = computed(() => {
  ...
  if (isDisasterChoice.value) return prompt.value?.playerIndex ?? null
  if (['disaster-ban', 'disaster-pick'].includes(waitingPrompt.value?.kind ?? ''))
    return waitingPrompt.value?.playerIndex ?? null
  return null
})
function setupRoleLabel(playerIndex: number | null) {
  ...
  <Teleport to="body">
    <div v-if="visible" class="l12-prompt-overlay"
      ...
      <section v-if="minimized" class="prompt-minimized-bar" role="status">
        ...
        <SetupDecisionClock :player-index="setupClockPlayerIndex" :ranked-clock="l12State.rankedClock"
...
.all-placement-workspace{display:grid;grid-template-columns:62px minmax(0,1fr) 62px;align-items:center;gap:8px;margin:10px 3px;padding:10px;border:1px solid rgba(238,238,228,.28);background:#090d0e}.all-placement-row{min-width:0;padding:5px}.placement-edge{color:#fff;font-size:max(18px,var(--l12-board-readable,14px));font-weight:900;letter-spacing:.28em;text-align:center;writing-mode:vertical-rl}.top-edge{color:#70d7df}.bottom-edge{color:#d76069}
.prompt-card-detail{display:grid;min-height:112px;grid-template-columns:130px 1fr;gap:14px;margin:0 3px 10px;padding:10px;border:1px solid rgba(238,238,228,.32);background:#090d0e}.prompt-card-detail>.l12-card-image{width:130px;height:108px;background:#050708}.prompt-card-detail small{color:#70d7df;font-size:max(14px,var(--l12-board-readable,14px));letter-spacing:.12em}.prompt-card-detail h3{margin:3px 0 5px;color:#fff;font-size:max(16px,var(--l12-board-readable,14px))}.prompt-card-detail dl{display:flex;gap:12px;margin:0}.prompt-card-detail dl div{display:flex;gap:4px}.prompt-card-detail dt{color:#777f7c;font-size:max(14px,var(--l12-board-readable,14px))}.prompt-card-detail dd{margin:0;color:#fff;font-size:max(14px,var(--l12-board-readable,14px));font-weight:900}.prompt-card-detail p{max-height:48px;margin:5px 0 0;overflow:auto;color:#c9cdc7;font-size:max(14px,var(--l12-board-readable,14px));font-weight:800;line-height:1.55;white-space:pre-wrap}
.prompt-panel footer button.primary{border:2px solid #fff!important;background:#f2eee3!important;color:#090c0d!important}.prompt-panel footer button.primary:disabled{border-color:#646966!important;background:#2a2e2d!important;color:#929792!important}.l12-prompt-overlay.waiting{background:rgba(2,4,5,.3)!important;backdrop-filter:blur(2px)}.waiting-panel{position:relative;width:min(430px,calc(100vw - 32px));padding:25px;text-align:center}.waiting-panel>.prompt-minimize{right:10px;top:10px}.waiting-panel small{color:#73d7de;font-size:max(14px,var(--l12-board-readable,14px));letter-spacing:.16em}.waiting-panel h2{margin:10px 0;color:#fff;font-size:max(21px,var(--l12-board-readable,14px))}.waiting-panel p{color:#c0c5bf;font-size:max(14px,var(--l12-board-readable,14px))}.waiting-panel i{display:inline-block;width:7px;height:7px;margin:10px 4px 0;border-radius:50%;background:#70d7df;animation:waiting-pulse 1.2s infinite}.waiting-panel i:nth-of-type(2){animation-delay:.2s}.waiting-panel i:nth-of-type(3){animation-delay:.4s}@keyframes waiting-pulse{0%,70%,100%{opacity:.25;transform:translateY(0)}35%{opacity:1;transform:translateY(-4px)}}
.l12-prompt-overlay.minimized{z-index:2147483600;inset:auto 16px 66px auto;width:auto;height:auto;padding:0;background:transparent!important;backdrop-filter:none;pointer-events:none}.prompt-minimized-bar{display:grid;gap:6px;pointer-events:auto}.prompt-minimized-bar button{padding:7px 12px;border:1px solid #70d7df;background:#174e54;color:#fff;font-size:max(14px,var(--l12-board-readable,14px));font-weight:900;box-shadow:0 12px 35px #000}
...
@media(max-width:700px){.placement-workspace{grid-template-columns:1fr;max-height:430px;overflow:auto}.all-placement-workspace{grid-template-columns:44px minmax(0,1fr) 44px}.placement-edge{font-size:max(14px,var(--l12-board-readable,14px))}}
.l12-prompt-overlay.preparation .prompt-panel{width:min(1100px,calc(100vw - 36px))}
.disaster-preparation-history{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:10px 0 12px;padding:9px;border:1px solid #3c4646;background:#080c0d;text-align:left}.disaster-preparation-history>section{min-width:0;border:1px solid #394240;background:#101516}.disaster-preparation-history>section>header{display:flex;align-items:center;justify-content:space-between;padding:6px 8px;border-bottom:1px solid #333c3a}.disaster-preparation-history>section>header b{font-size:max(14px,var(--l12-board-readable,14px))}.disaster-preparation-history>section>header span{display:grid;width:19px;height:19px;place-items:center;background:#232a28;color:#fff;font:900 max(14px,var(--l12-board-readable,14px)) monospace}.disaster-preparation-history>section>div{display:flex;min-height:78px;align-items:center;gap:5px;overflow-x:auto;padding:6px}.disaster-preparation-history button{display:flex;width:112px;min-width:112px;flex-direction:column;gap:3px;padding:3px;border:2px solid #68706d;background:#080b0c;color:#fff}.disaster-preparation-history img,.disaster-preparation-history .l12-card-image{width:102px;height:auto;aspect-ratio:8/5}.disaster-preparation-history button span{overflow:hidden;font-size:max(14px,var(--l12-board-readable,14px));font-weight:900;text-overflow:ellipsis;white-space:nowrap}.disaster-preparation-history p{margin:auto;color:#626b68;font-size:max(14px,var(--l12-board-readable,14px))}.disaster-preparation-history .banned button{border-color:#a83c46;box-shadow:0 0 8px rgba(168,60,70,.22)}.disaster-preparation-history .banned>header b{color:#ef7780}.disaster-preparation-history .revealed button{border-color:#d1b76c;box-shadow:0 0 8px rgba(209,183,108,.2)}.disaster-preparation-history .revealed>header b{color:#e8cf83}.disaster-preparation-history .chosen button{border-color:#3f9d73;box-shadow:0 0 8px rgba(63,157,115,.24)}.disaster-preparation-history .chosen>header b{color:#79d2a7}.l12-prompt-overlay.preparation .waiting-panel{width:min(1100px,calc(100vw - 36px));padding:16px 20px 22px}.l12-prompt-overlay.preparation .waiting-panel>small{display:block;margin-top:12px}
.disaster-preparation-history{grid-template-columns:minmax(260px,.8fr) minmax(420px,1.2fr)}.disaster-preparation-history button small{display:block;width:100%;overflow:hidden;color:#9ca6a1;font-size:max(14px,var(--l12-board-readable,14px));font-weight:900;text-align:center;text-overflow:ellipsis;white-space:nowrap}.disaster-preparation-history .chosen button small{color:#78d1a6}

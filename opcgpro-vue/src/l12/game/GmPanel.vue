...
function addCard() { run('addCard', { cardId: cardId.value.trim().toUpperCase(), destination: destination.value, value: count.value }) }
function placeCard() { run('placeCard', { cardId: cardId.value.trim().toUpperCase(), row: row.value, slot: slot.value, triggerEffects: triggerEffects.value }) }

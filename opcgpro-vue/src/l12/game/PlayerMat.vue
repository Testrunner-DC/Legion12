...
function cavalryMoveActionLabel(card: Card) {
  const label = cavalryMoveAction(card)?.label?.split(/[：:]/, 1)[0]?.trim()
  return label || '骑兵位移'
}
      ...
      <div v-if="player.faction === 'otherworld' || player.specialZones?.canopicTrack?.length"
        ...
        <template v-else>
          <button v-for="card in player.specialZones?.canopicTrack ?? []" :key="card.cardId" type="button"
            ...
            <RoundCardImage :card-id="card.cardId" :legacy-url="card.imageUrl" />
          </button>
        </template>
      </div>
            ...
            <template v-if="player.field[row][slot]">
              ...
              <div v-if="selectedId === player.field[row][slot]!.instanceId && actionsEnabled && !attackMode && !moveMode && !freeMoveMode && !cavalryMoveMode && (canUseAbilities(player.field[row][slot]!) || canTrial(player.field[row][slot]!))"
                ...
                <button v-if="canUseAbilities(player.field[row][slot]!) && hasCavalryMoveAction(player.field[row][slot]!)" :class="{ active: cavalryMoveMode }"
                  ...
                  @click.stop="emit('cardAction', 'cavalryMove', player.field[row][slot]!)">{{ cavalryMoveMode ? '选择任意位置' : cavalryMoveActionLabel(player.field[row][slot]!) }}</button>
...
.canopic-orb{width:36px;height:36px;min-width:36px;padding:0;overflow:hidden;border:1px solid #63555a;border-radius:50%;background:#090a0b;filter:grayscale(1) brightness(.3);cursor:pointer}.canopic-orb :deep(.l12-round-card-image){width:100%;height:100%;transform:scale(1.12)}.canopic-orb.completed{border-color:#d0aa52;filter:none;box-shadow:0 0 7px rgba(208,170,82,.65)}.canopic-orb.activatable{border-color:#6ee2a0;box-shadow:0 0 9px rgba(82,213,138,.8);animation:canopic-ready 1.25s ease-in-out infinite alternate}@keyframes canopic-ready{to{transform:translateY(-2px);filter:brightness(1.18)}}
...
.l12-player-mat{grid-template-columns:minmax(270px,300px) minmax(500px,1fr) 100px 156px}.mat-piles{transform:none}
...
.commander-zone{grid-template-columns:140px 132.25px}.battle-zone{transform:none}.mat-piles{transform:none}

...
defineProps<{
  game: GameState; me: PlayerView; mode: 'play' | 'attack' | 'move'; selectedId: string | null;
...
<template>
  <div class="l12-actions">
    ...
    <template v-else-if="game.phase === 'Defense' && game.activePlayer !== game.you && defenseTargetType === 'master'">
    ...
    <template v-else-if="game.phase === 'Defense' && game.activePlayer !== game.you && defenseTargetType === 'legion'">
    ...
    <template v-else-if="game.activePlayer === game.you && ['Disaster','Reset','Draw','Morale','End'].includes(game.phase)">
      <p>服务器正在依次执行阶段步骤…</p>
    </template>
    <template v-else-if="game.activePlayer === game.you && game.phase === 'Main'">

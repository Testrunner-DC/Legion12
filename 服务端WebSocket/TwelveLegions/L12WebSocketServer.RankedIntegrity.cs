...
public sealed partial class L12WebSocketServer
{
    ...
    {
        var app = _app ?? throw new InvalidOperationException("WebSocket server app is not initialized");
        app.MapPost("/api/admin/ranked/integrity/preview", (HttpRequest request, L12RankedIntegrityActionInput input) =>
        {
        ...
        app.MapGet("/api/ranked/integrity/notifications", (HttpRequest request, string? cursor, int? limit, bool? unreadOnly) =>
        {
        ...
        app.MapPost("/api/ranked/integrity/notifications/{id}/ack", (HttpRequest request, string id) =>
        {
            var account = _platform.Authenticate(request.Headers.Authorization);
            if (account is null) return Results.Unauthorized();
            return IntegrityRequest(request, () => { _platform.AcknowledgeRankedIntegrityNotification(account, id); return new { acknowledged = true }; });
        });
        app.MapPost("/api/ranked/integrity/appeals", (HttpRequest request, IntegrityAppealRequest body) =>
        {
            var account = _platform.Authenticate(request.Headers.Authorization);
            if (account is null) return Results.Unauthorized();
        ...
        app.MapGet("/api/ranked/integrity/appeals", (HttpRequest request, string? cursor, int? limit) =>
        {
            var account = _platform.Authenticate(request.Headers.Authorization);
            if (account is null) return Results.Unauthorized();
            return IntegrityRequest(request, () => _platform.RankedIntegrityAppealsForPlayer(account, cursor, limit ?? 20));
        });
        app.MapGet("/api/admin/ranked/integrity/appeals", (HttpRequest request, string? decisionId, string? status, string? cursor, int? limit) =>
        {
            if (!TryAuthorize(request, L12Permission.AdminAuditRead, out var auth, out var failure)) return failure;

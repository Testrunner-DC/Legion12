...
public sealed partial class L12GameEngine
{
        ...
        switch (card.CardId)
        {
            case "S01-0221":
                CreatePrompt(item.Controller, "option", "杜阿特之门：选择击杀军团，或回收太阳城卡牌", ["kill", "recover"], 1, 1,
                    "card-effect", item.StackItemId, data: new Dictionary<string, string> { ["action"] = "duat-mode" }); return true;
    ...
    private bool TryResolveS1FactionDeath(L12StackItem item, L12CardInstance card)
    {
        ...
        switch (card.CardId)
        {
            case "S01-0201": ApplySunKingAttack(item); return true;
            case "S01-0204":
                foreach (var guard in card.AttachedCards.ToArray()) { card.AttachedCards.Remove(guard); player.Graveyard.Add(guard); }
    ...
    private bool TryContinueS1Faction(L12StackItem item, L12Prompt prompt, List<string> chosen, L12Command command)
    {
        ...
        switch (action)
        {
            ...
            case "nefertiti-discard": MoveHandToGrave(State.Players[prompt.PlayerIndex], chosen[0]); FinishStackItem(item); return true;
            ...
            case "death-cycle-discard": MoveHandToGrave(player, chosen[0]); FinishStackItem(item); return true;
            ...
            case "erik-discard": MoveHandToGrave(State.Players[prompt.PlayerIndex], chosen[0]); FinishStackItem(item); return true;
            case "palace-kill":
            {
            ...
            case "mengpo-discard": MoveHandToGrave(player, chosen[0]); AddMorale(player, 1, true); FinishStackItem(item); return true;
            ...
            case "amaterasu-discard":
                MoveHandToGrave(player, chosen[0]); if (source is not null) foreach (var morale in player.Morale.Where(card => card.Tapped).Take(2).ToArray()) ReadyMoraleByEffect(item.Controller, source, morale, "士气因效果转为活跃");
    ...
    private CommandResult? TryCommitS1FactionActiveAbility(int playerIndex, L12CardInstance source, string ability, string? target, string onceKey, bool? useTombGuards)
    {
        ...
        switch (ability)
        {
            ...
            case "valhallaKill" when source.CardId == "S01-03D1": if (source.Tapped || player.Graveyard.Count < 2) return CommandResult.Reject("英灵殿需为活跃，墓地需至少2张牌"); source.Tapped = true; break;
            ...
            case "yomiRecover" when source.CardId == "S01-04D1": if (source.Tapped) return CommandResult.Reject("黄泉之门必须为活跃状态"); source.Tapped = true; break;
    ...
    private bool TryResolveS1FactionActive(L12StackItem item, L12CardInstance? source, string ability)
    {
        ...
        switch (ability)
        {
            ...
            case "medjedDebuff": ApplyDeclaredTroopsDelta(item, -1000); FinishStackItem(item); return true;
    ...
    private bool TryPreventS1FactionDeath(L12PlayerState player, L12CardInstance card)
    {
        ...
        if (FindOnField(player, guard.InstanceId, out var row, out var slot) is not null) { player.Field[row][slot] = null; player.Graveyard.Add(guard); }
    ...
    {
        if (row != 0 || State.ActivePlayer == defender.PlayerIndex) return;
        if (target.CardId is not ("S01-0212" or "S01-0312")) return;
        var key = $"defense-buff:{target.InstanceId}:{State.TurnSerial}";
        if (defender.UsedAbilities.Add(key)) { target.Troops += 1000; AddEvent("effect", defender.PlayerIndex, $"{target.Name}在对方回合位于前排，兵力+1000", target); }
    }
    ...
    private void CompleteFactionTopSearch(L12StackItem item, List<string> chosen)
    {
        ...
        CompleteFactionSearchOrder(item, remaining);
    }
    ...
    {
        var card = FindOnField(player, instanceId, out var row, out var slot); if (card is null) return; var (targetRow, targetSlot) = ParseSlot(slotChoice); player.Field[row][slot] = null; player.Field[targetRow][targetSlot] = card;
    }
}

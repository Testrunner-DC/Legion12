...
readonly source_snippet="${1:-/tmp/nginx-l12-web-assets.conf}"
readonly target_snippet="/etc/nginx/snippets/legion12-web-assets.conf"
readonly production_site="/etc/nginx/sites-available/legion12"
readonly production_enabled="/etc/nginx/sites-enabled/legion12"
readonly include_line="    include /etc/nginx/snippets/legion12-web-assets.conf;"

fail() { printf '[L12 web assets] ERROR: %s\n' "$*" >&2; exit 1; }
...
grep -Fq 'location ^~ /assets/' "$source_snippet" || fail "strict asset route is missing"
grep -Fq 'root /opt/legion12-web-assets;' "$source_snippet" || fail "asset route does not use the managed compatibility pool"
grep -Fq 'try_files $uri =404;' "$source_snippet" || fail "asset route does not fail closed"

readonly suffix="$(date -u +%Y%m%dT%H%M%SZ)-$$"
readonly site_backup="/etc/nginx/sites-available/.legion12-before-web-assets-${suffix}"
readonly snippet_backup="/etc/nginx/snippets/.legion12-web-assets-before-${suffix}.conf"
snippet_existed=0
completed=0
...
if [[ -f "$target_snippet" && ! -L "$target_snippet" ]]; then
  ...
  fail "installed web asset snippet is unsafe"
fi
...
rm -f "$site_backup" "$snippet_backup" "$source_snippet"
trap - EXIT ERR INT TERM
printf '[L12 web assets] strict immutable asset routes are active\n'

...
- 克劳迪娅等冒号前费用在声明提交时支付，被无效或目标失效不返还。诸葛亮、卡诺匹斯罐和武田信玄的后续段独立入栈；卡诺匹斯无首段目标时直接从弃置段开始，武田拒绝检索时直接建立真田登场后段声明，强制目标不存在时不留下空栈、空Prompt或悬挂candidate。
...
- 固定清单冻结53张124项能力；`AtomicReviewBatch6KBRegressionTests`覆盖图特摩斯进攻/阵亡、伊瓦尔登场、纳芙蒂蒂受影响方私密弃牌、卡诺匹斯箱/罐二/罐三与众神之乡/英灵殿的公开声明、隐藏信息和独立段。逐卡审计表必须恰有53张唯一状态：43通过、8明确错误已修复、2有疑点。
...
- 固定清单冻结35张101项能力；`AtomicReviewBatch6LCRegressionTests`覆盖月读后→前独立触发、武运在天隐藏检索/确定性后段、晋升状态继承、万物统御之戒跨区域奥林匹斯筛选、诸神巅独立回收/登场及神力四状态。逐卡审计表必须恰有35张唯一状态：20通过、14明确错误已修复、1有疑点。

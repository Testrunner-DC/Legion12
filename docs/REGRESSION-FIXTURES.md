...
- 固定种子`11001`至`11004`覆盖哮天犬·稚无士气牌库静默、阿塔兰忒唯一士气仍需点击、声明目标在响应后成为神力而结算失败、哮天犬被无效不追加士气，以及V2响应中恢复后恰一次结算与旧Prompt拒绝。
- 规格清单固定9项：哮天犬·稚、阿塔兰忒、阿尔忒弥斯、莫瑞甘、李牧、〈寻找圣杯之旅〉、安格斯、孙悟空、天廷士气。每项必须只有一个`single-trigger`实际结算场景；直接单段能力必须生成可执行原子程序，孙悟空则显式绑定离场返回能力的资源子段。

...
- NewSystemsTests 固定后续回合（非首回合）自然增长到阈值翻开〈诸神黄昏〉：来源必须是 turn-phase，双方各抽2后继续当前回合的重置、抽牌、士气流程，不追加回合。
- 带天灾等级军团登场跨阈值的样例固定 card-effect，在延后堆叠关闭、V2检查点恢复后仍保留该来源；〈诸神黄昏〉必须走主动分支。GM触发另记 gm，不得测试成玩家卡效。
- 旧V1 Stack只有 opening 时按原值回退；新V2必须优先读取 triggerSource，禁止再按回合号推断。

### 支援阵亡与进攻方击杀

- `CombatTimelineRegressionTests`固定源义经进攻、武则天支援：支援者按阵亡离场，武则天的抽牌与治疗完成；但不产生`kill-source`，源义经不建立【击杀时】。
- 在支援声明后的响应链中写入V2检查点并恢复，后续响应只继续一次支援死亡触发链；不得重复阵亡、重复抽牌/治疗，或在恢复后补建进攻方击杀。

### 进攻时自身单段兵力增益

- `SimpleSelfTroopBuffTriggerConsistencyTests`固定贝奥武夫、古斯塔夫一世、奥德修斯、彭忒西勒亚、斯巴达勇士、帕西瓦尔6项，种子11200至11404覆盖逐卡正例、不可支付、错误费用选择、取消、无效、来源离场、重复提交和V2恢复。
- 每项必须先完成真实冒号前Cost，再开放响应；细原子中的`prepaid=true`费用节点只作为结构事实，不得在结算期重复扣除。响应文本同时包含实际`Cost（已支付）`和本次兵力效果。
- 来源在响应后不位于控制者战场时必须发布`failed`且不得修改冻结快照；被无效必须发布`negated`且保留已付Cost；成功只添加一条以规格卡名为来源、到本回合结束失效的兵力修正。

### 单目标军团状态触发

- `SimpleCardStateTriggerConsistencyTests`固定尼托克丽丝、神箭奥德尔、疯狂的爱丽丝、安提诺乌斯4项规格。战场目标即使唯一也人工点击；固定“此军团”不重复选目标。覆盖正例、无候选静默、选发拒绝、重复提交、V2响应恢复、被无效和逆序失效不改选。
- 爱丽丝样例必须携带权威`combatKillConfirmed`且来源当前休整，并验证pending→final回合一次；安提诺乌斯必须先存在本回合主宰弃牌事实，目标只允许当前有效【奥林匹斯】军团。
- 转活跃样例包含两层响应窗口：卡牌效果声明完成后先结算到`effect-ready`权威事件，再在该事件响应中令目标离区或已活跃。最终必须保持现状态、记录`failed`且不得先发布同场景`resolved`。
...
- 固定种子`11001`至`11007`覆盖哮天犬·稚无士气牌库静默、阿塔兰忒唯一士气仍需点击、声明目标在响应后成为神力而结算失败、哮天犬被无效不追加士气、V2响应中恢复后恰一次结算与旧Prompt拒绝，以及登场得符文的必发无空声明、选发拒绝和7卡逐卡结算。
- 规格清单固定16项：梅林、帕西瓦尔、高文、阿麦金、伊丽莎白·都铎、加雷斯、纯白的灵鹿，以及哮天犬·稚、阿塔兰忒、阿尔忒弥斯、莫瑞甘、李牧、〈寻找圣杯之旅〉、安格斯、孙悟空、天廷士气。每项必须只有一个`single-trigger`实际结算场景；直接单段能力必须生成可执行原子程序，孙悟空则显式绑定离场返回能力的资源子段。
- 士气翻转目标必须在入栈前由玩家选择，结算时仍需位于控制者士气区且不是神力；回合一次在声明提交时消耗，目标/条件逆序失效及被无效均不恢复次数。无合法对象的效果在候选阶段静默，不生成空声明或悬挂堆叠。
- 必发且没有模式、目标或费用的资源效果必须直接进入响应堆叠，不能建立零步骤`PendingActivation`；选发资源效果拒绝后不生成StackItem。该不变量适用于后续新增的同型资源能力。
...
- 固定种子 `7621` 至 `7626` 覆盖寻找圣杯/湖中仙女的牌库身份延迟、湖中仙女首段拒绝时直接从强制回牌库/重洗段入栈而不产生空响应栈、该强制段被无效后费用减免段仍继续、芬尼亚每次1符文/1公开目标/1独立堆叠、目标失效费用不退、离栈后重复询问与同目标再次选择、安格斯同一时点排序/独立可选触发，以及十字军东征无印刷完成触发控制组。
- 芬尼亚不再使用`rune-count`与`target1..N`批量协议；每次声明只保存`target`，支付1符文后独立入栈，完整离栈且仍有资源/目标时重新建立可选候选。湖中仙女使用显式 `mode:none` 跳过首句，整次声明不提供会吞掉后续强制段的 `skip`。

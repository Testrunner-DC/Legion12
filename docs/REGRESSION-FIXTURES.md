...
- 固定种子 `307108` 至 `307318` 覆盖成功、被无效、无对象跳过、失败、选择不发动与入栈前不能发动；声明和结果必须携带同一场景、能力、效果段及公开分支，结构化声明不得重复进入卡牌动效。
- 同一StackItem重复尝试发布结果时只允许一个 `effect-result`；`effectResultPublished` 是随检查点保存的去重凭据。未完成结算的 `effectResultStatus` 与 `presentationSceneId` 在格式2检查点恢复后必须保持，随后用原冻结场景结算。
- 真实《万箭齐发》前排分支分别固定多行动效文本、最终成功结果及空前排时可发动但结果为跳过；取消分支不入栈、不发布结果。旧无 `Flow` 的整卡场景是明确豁免，不得靠补空元数据冒充迁移。
...
- 固定种子 `7621` 至 `7626` 覆盖寻找圣杯/湖中仙女的牌库身份延迟、湖中仙女首段拒绝时直接从强制回牌库/重洗段入栈而不产生空响应栈、该强制段被无效后费用减免段仍继续、芬尼亚无冒号的X符文与全部减兵力目标作为单一效果链、被无效不消耗符文、来源离区或任一目标失效时整链失败、安格斯同一时点排序/独立可选触发，以及十字军东征无印刷完成触发控制组。
...
- 梅林和圣杯在StackItem合法开始前只能读取公开牌库数量，不得把匹配卡实例、卡号或“是否命中”写入声明；戒指通用卡应被彼界效果识别。阿麦金和十字军“只拥有/只有彼界特征”统一读取有效特征集合：六阵营/通用是基础特征，圆桌骑士、晋升者及“某主宰专属”是附加特征，职介和试炼值不计入；万物统御之戒把通用基础特征替换为主宰阵营特征。

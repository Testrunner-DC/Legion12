...
`archive\legacy` 中的旧克隆保留原分支和未提交内容。确认不再需要前，不应手工删除。迁移前的主仓库和权威工作树恢复包位于 `migration\compact-snapshots`；它们只保存当时未提交的文件，已提交内容从清单记录的 Git HEAD 恢复，避免重复保存数 GiB 的构建输出和 Git 对象。

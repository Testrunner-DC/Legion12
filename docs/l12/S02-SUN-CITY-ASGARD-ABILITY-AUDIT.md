...
| S02-0301 雷神之锤 | 4 | 登场仅获得本回合可攻主宰；阵亡发动模式先声明，抽后弃牌身份延迟；墓地主动按钮即发动意图，有序3张其他墓地费用与公开位置提交后原子回底并入栈，第1/1段统一记录实际登场结果。 | `L12PublicTriggerEffectPlans`、`L12S2RemainingEffects`、`L12SingleSegmentEffectPresentations` | `AtomicReviewBatch6IBRegressionTests`、`AtomicReviewBatch6LBRegressionTests`、`Bq20260905_01RegressionTests`、`S2FactionRegressionTests` | 明确错误→已修复 |
...
| S02-0304 玛格丽特一世 | 3 | 远程静态；登场弃顶模式候选期声明；受主宰效果伤害使用`master-damaged-by-effect`，休整费用预付，治疗与随后禁疗是两个独立响应段。 | `L12PublicTriggerEffectPlans`、`L12CompositeEffectPlans` | `AtomicReviewBatch6GARegressionTests`、`S2FactionRegressionTests` | 通过 |
| S02-0305 安德华拉诺特 | 4 | 主宰伤害抽牌使用`master-damaged`单段触发，放弃/成功/无效/恢复同源；开局、弃至6、圣物禁打与首伤改2保持独立。 | `L12SingleSegmentTriggeredEffectPresentations`、`L12PublicTriggerEffectPlans`、`L12GameEngine` | `AtomicReviewBatch6GARegressionTests`、`S2FactionRegressionTests` | 通过 |

...
- 固定范围为 `cards.s2.json` 的 32 张 `otherworld` 彼界卡与 6 张 `S02-DSxx` 天灾卡，含主宰、主神、士气/符文、试炼、王者之剑与特殊牌，共 38 张、108 项当前原子目录能力。其中 22 张圆桌/彼界主牌已按玩家人工定义拆为 74 项能力。
- 权威顺序为玩家裁定 > `FAQ-RULINGS.md`/FAQ > 规则书与关键词 > 印刷卡面。逐项核对试炼发动/推进/完成、费用预付、独立段、区域/所有者、符文、隐藏信息、来源 LKI、天灾不可响应与响应无效。
- 唯一结论：30 张通过、8 张明确错误并修复、有疑点 0、缺少测试 0、未实现 0。至此 S1+S2 全 248 张、577 项目录能力均有逐卡唯一结论；2026-09-13 补齐诸神巅此前漏列的开场能力后曾为578项，本次删除〈无眠之夜〉把“主动休整时”误切出的虚假主动能力后回到577项。
...
| S02-0603 梅林 | 5 | 无法进攻；登场必发得符文由统一资源协议直接入响应堆叠且无空声明；主动只先声明公开模式/敌军目标并预付符文休整，牌库命中身份和存在性到合法结算期私密选择。 | `L12SimpleResourceTriggerEffects`、`L12PublicTriggerEffectPlans`、`L12S2FactionEffects`、`L12StructuredCardRules` | `SimpleResourceTriggerConsistencyTests`、`AtomicReviewBatch6LDRegressionTests` | 明确错误→已修复 |
...
| S02-0606 帕西瓦尔 | 5 | 独立保留试炼1、贯穿定义；登场必发得符文走统一资源协议；进攻弃手牌预付获+2000；真实击杀后本回合获得贯穿。 | `L12SimpleResourceTriggerEffects`、`L12SimpleSelfTroopBuffTriggerEffects`、`L12AttackPublicTriggerPlans`、`L12AtomicRuntimeIntegration`、`L12KillSourceEvents` | `SimpleResourceTriggerConsistencyTests`、`SimpleSelfTroopBuffTriggerConsistencyTests`、`S02OtherworldHumanAssistedAtomicTests`、`AtomicReviewBatch6HRegressionTests`、`LatestBugRegressionTests` | 通过 |
| S02-0607 高文 | 2 | 登场必发得符文走统一资源协议；进攻无冒号的X符文变化在结算期整体执行，符文消费与本回合后续全部进攻增伤为独立段。 | `L12SimpleResourceTriggerEffects`、`L12AttackPublicTriggerPlans` | `SimpleResourceTriggerConsistencyTests`、`AtomicReviewBatch6HRegressionTests` | 通过 |
...
| S02-0616 阿麦金 | 3 | 休整时保护活跃试炼军团；登场得符文由统一资源协议选发，拒绝不入栈；主动休整后才展示牌库顶；“只拥有彼界特征”读取共享有效特征集合，戒指转换后的纯通用卡可成为唯一彼界，附带圆桌骑士/晋升者/专属特征则不可。 | `L12SimpleResourceTriggerEffects`、`L12PublicTriggerEffectPlans`、`L12S2FactionEffects`、`L12StructuredCardRules` | `SimpleResourceTriggerConsistencyTests`、`S2FactionRegressionTests` | 通过 |
...
| S02-0618 伊丽莎白·都铎 | 3 | 独立保留试炼2与远程职介能力；登场得符文为统一资源协议必发触发，直接进入响应堆叠且不创建空声明。 | `L12SimpleResourceTriggerEffects`、`L12PublicTriggerEffectPlans` | `SimpleResourceTriggerConsistencyTests`、`S02OtherworldHumanAssistedAtomicTests`、`S2FactionRegressionTests` | 通过 |
...
| S02-06C1 士气 | 1 | 阵营点击回合1次支付2士气得符文；裸“阵营效果”标签已并入该真实能力，不再单独计段。 | `L12S2FactionEffects` | `AtomicReviewBatch6FRegressionTests`、`S2FactionRegressionTests` | 通过 |
| S02-06D1 彼界 阿瓦隆 | 5 | 可携带完成试炼；回合开始推进+得符文为确定性不可拒绝事件；回收与随后免费战术独立；主动休整公开目标先声明；主城共用开场追加2张士气。 | `L12TrialAdvanceEffectPlans`、`L12PublicActiveEffectPlans`、`L12CompositeEffectPlans`、`SetupPlayers` | `AtomicReviewBatch6FRegressionTests`、`AtomicReviewBatch2RegressionTests`、`GameSetupLifecycleProfileTests` | 通过 |
| S02-06M1 莫瑞甘 | 2 | 对方阵亡得符文与主动均各自回合1次；主动符文预付且戒指通用军团可作彼界公开目标。 | `L12SimpleResourceTriggerEffects`、`L12PublicTriggerEffectPlans`、`L12S2FactionEffects` | `SimpleResourceTriggerConsistencyTests`、`AtomicReviewBatch6GARegressionTests`、`AtomicReviewBatch6LDRegressionTests` | 明确错误→已修复 |
| S02-06M2 安格斯·麦·奥格 | 3 | 可完成试炼数量+1、推进试炼得符文、战术成功强制推进是三个独立能力；后两者分别使用`trial-advance`与`tactic-effect-resolved`，推进不等于完成。 | `L12SimpleResourceTriggerEffects`、`L12TrialCompletionTriggerPlans`、`L12TrialAdvanceEffectPlans` | `SimpleResourceTriggerConsistencyTests`、`AtomicReviewBatch6BRegressionTests`、`AtomicReviewBatch6FRegressionTests` | 通过 |
...
| S02-06S4 寻找圣杯之旅 | 2 | 完成检索与圆桌登场得符文是两个真实能力，不再把句内“登场时”误拆成第三段；后者使用`friendly-round-table-enter`单段触发并保持独立once。 | `L12SimpleResourceTriggerEffects`、`L12TrialCompletionTriggerPlans`、`L12SingleSegmentTriggeredEffectPresentations`、`L12PublicTriggerEffectPlans` | `SimpleResourceTriggerConsistencyTests`、`AtomicReviewBatch6BRegressionTests`、`AtomicReviewBatch6GARegressionTests`、`AtomicReviewBatch6LDRegressionTests` | 明确错误→已修复 |
| S02-06S5 芬尼亚传奇 | 2 | 完成触发每次消耗1符文并选择1张对方军团，形成一个独立响应堆叠；该堆叠完整离栈后才询问是否重复，同一仍合法目标可再次选择。主动转活跃保持原有效彼界判定与回合1次。 | `L12TrialCompletionTriggerPlans`、`L12S2FactionEffects` | `AtomicReviewBatch6BRegressionTests`、`AtomicReviewBatch6LDRegressionTests` | 明确错误→已修复 |
...
| S02-DS03 无眠之夜 | 2 | 仅含天灾触发与持续监听两项；“主动休整时”不是本卡的主动按钮。触发弃置原本兵力≤2000军团，持续项仅在玩家实际使用结构化主动休整时对其主宰造成中立非致命伤害。 | `L12Disasters`、`L12StructuredCardRules`、`L12PromptsAndSetup` | `AtomicReviewBatch6LDRegressionTests`、`NewSystemsTests`、`S2DisasterLevelRegressionTests` | 通过 |
...
- 2026-09-04 人工拆分后，22 张圆桌/彼界主牌由旧机械边界 49 项重构为 74 项，独立保留试炼值、授予能力、关键词定义、复合句独立段及手牌条件；相关实战入口继续复用既有公共计划，不恢复单卡结算旁路。专项人工原子与结构化查询 19/19、彼界/试炼/进攻兼容合并 275/275，Focused、Batch、Release 完整规则均 2245/2245 通过；Release 平台持久化 62/62、UI 契约 217 项、卡图契约 28 项/324 张及生产构建通过；当时38 卡/108 项、全池 248 张/577 项唯一审计门禁与公共声明门禁通过。2026-09-13补齐诸神巅漏列能力后短暂为578项；同日校正〈无眠之夜〉虚假主动拆分后，当前仍为577项。

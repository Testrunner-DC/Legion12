...
- 固定范围为 `cards.s2.json` 的 7 张 `gaotianyuan` 高天原卡与 28 张 `olympus` 奥林匹斯卡，含主宰、主神、士气/神力、晋升者与特殊牌，共 35 张、105 项原子目录能力；S02 天灾留后批。
...
| S02-0509 奥德修斯 | 3 | 0神力减费实时计算；登场免费战术计数只作用下一张；进攻展示的私密手牌战术在声明期仅对选择者可见。 | `L12Actions`、`L12EnterPublicTriggerPlans`、`L12SimpleSelfTroopBuffTriggerEffects`、`L12AttackPublicTriggerPlans`、`L12AtomicRuntimeIntegration` | `SimpleSelfTroopBuffTriggerConsistencyTests`、`AtomicReviewBatch6HRegressionTests`、`AtomicReviewBatch6JARegressionTests` | 通过 |
| S02-0510 希波吕忒 | 3 | 神力≥5减费与休整时免费前后位移持续生效；主动休整/3士气/弃牌在入栈前支付，墓地奥林匹斯目标与位置前置并按戒指有效阵营重验；第1/1段记录成功、失效或被无效。 | `L12S2RemainingEffects`、`L12StructuredCardRules`、`L12SingleSegmentEffectPresentations` | `AtomicReviewBatch6LCRegressionTests`、`S2FactionRegressionTests` | 明确错误→已修复 |
...
| S02-0517 彭忒西勒亚 | 3 | 前排远程常驻；登场本回合可攻军团；进攻消耗并翻转1神力在入栈前支付，+2000本回合有效且无效不退款。 | `L12SimpleSelfTroopBuffTriggerEffects`、`L12AttackPublicTriggerPlans`、`L12AtomicRuntimeIntegration`、`L12EnterPublicTriggerPlans` | `SimpleSelfTroopBuffTriggerConsistencyTests`、`AtomicReviewBatch6HRegressionTests`、`AtomicReviewBatch6JARegressionTests` | 通过 |
...
| S02-0519 斯巴达勇士 | 2 | 进攻消耗并翻转1神力在入栈前支付并获得+2000；对方回合+2000为持续层，修正标签不得冒充阿喀琉斯。 | `L12SimpleSelfTroopBuffTriggerEffects`、`L12AttackPublicTriggerPlans`、`L12AtomicRuntimeIntegration`、`L12StructuredCardRules` | `SimpleSelfTroopBuffTriggerConsistencyTests`、`AtomicReviewBatch6HRegressionTests`、`S2FactionRegressionTests` | 通过 |
...
| S02-0523 特洛伊木马 | 2 | 对方进攻后模式与对方战场空位前置，逆序结算时位置重验不覆盖；置入为结构化第1/1段。下个己方回合结束按实例逐个进入不可响应的弃置第1/2段与依赖抽牌第2/2段，弃置失败停止随后抽牌；场上持续-1000仍由区域状态层计算。 | `L12PublicTriggerEffectPlans`、`L12CompositeEffectPlans`、`L12S2RemainingEffects` | `AtomicReviewBatch6ERegressionTests`、`S2FactionRegressionTests` | 通过 |
| S02-05M1 阿尔忒弥斯 | 4 | 远程阵亡翻士气使用`friendly-ranged-death`单段触发，与主动各自回合1次；主动可消耗1神力但不翻面，或弃置1手牌，目标为任意费用的我方【奥林匹斯】军团；强攻/震击分支与期限不变。 | `L12SingleSegmentTriggeredEffectPresentations`、`L12PublicTriggerEffectPlans`、`L12S2RemainingEffects` | `ArtemisActiveLifecycleTests`、`AtomicReviewBatch6GARegressionTests`、`AtomicReviewBatch6LCRegressionTests` | 明确错误→已修复 |
...
| S02-05D1 奥林匹斯 诸神巅 | 4 | 四项印刷能力均有结构定义。两项回合1次主动分别绑定第1、2段，主动休整免晋升神力绑定第3段，开场额外2士气为第4段。翻士气在结算时选择并在候选消失时`failed`；2神力预付后，回收/可选登场与六份伤害使用独立场景，伤害逐份重验，全部失效为`failed`、部分有效继续结算；明确不登场为独立`declined`，已付Cost不回退。 | `L12StructuredCardRules`、`L12CompositeEffectPlans`、`L12S2RemainingEffects`、`L12SingleSegmentEffectPresentations` | `DivinityEffectLifecycleTests`、`EffectPresentationBranchSegmentTests`、`AtomicReviewBatch6LCRegressionTests` | 明确错误→已修复 |

...
- 待裁定：托勒密十三世再次发动上一张主动战术时是否重付冒号前费用；只登记为隔离，不修改运行语义。
...
| `L12S1FactionEffects`托勒密十三世 | 待裁定 | 再次发动是否重付原战术冒号前费用未裁定，禁止猜测迁移 |
...
- `scripts/test-l12-effect-generated-play-transactions.ps1`锁定：直接play Push不高于3、Hand.Add不高于7、Library.Remove不高于41，并限定合法文件与旧令牌清零。

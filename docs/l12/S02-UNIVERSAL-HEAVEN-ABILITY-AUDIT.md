...
| S02-0104 神农鼎 | 2 | 登场抽牌可选模式在候选期声明；主动休整、确切1士气返还费用及公开主宰能力目标在入栈前提交，无效不返费；结算时所选次数已被上层响应重置则为`failed`。 | `L12PublicTriggerEffectPlans`、`L12PublicActiveEffectPlans`、`L12MoraleReturns`、`L12SingleSegmentEffectPresentations` | `AtomicReviewBatch6IARegressionTests`、`AtomicEffectsTests`、`SingleActiveTargetStatePresentationTests` | 通过 |

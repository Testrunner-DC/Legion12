...
## 2026-09-24 比例与停靠位置复核

- 主宰宽度成为移动战场唯一比例基准，并同时受逻辑画布宽、高约束。左栏宽度至少为主宰的 `130%`，且保留 78px 的天灾圆图可用下限；右栏内容宽度为主宰的 `118%`，外加 8px 战场预留。
- 圣物宽度固定为主宰的 `90%`，牌库与墓地固定为主宰的 `70%`。主宰周围的符文／卡诺匹斯圆图、手牌数量牌和其文字均按主宰宽度连续缩放。
- 三段快捷工具从右侧操作栏移到左栏最下方。左栏为其保留真实高度，天灾、试炼等内容不会被覆盖；安全区仍只由生产视口宿主计算一次。
- 双方信息永久展开，删除折叠状态和折叠按钮。当前卡牌动作在内容较少时以自动上边距贴住主操作区，始终直接位于“结束回合”上方；动作过多时只在当前操作区内部滚动。
- 我方手牌选中态取消向战场方向抬升，选择描边改为卡内描边，并为手牌轨保留上下内边距，避免被战场边框或手牌轨裁切。
- 长期浏览器守卫逐档核对左右栏／主宰、圣物／主宰、牌堆／主宰、手牌数量／主宰、圆图／主宰比例，并验证工具位置、信息常驻、动作顺序和选中手牌命中。

## 2026-09-24 互动区残留复核

- 移动右栏的身份常驻内容收敛为双方完整姓名，顺序固定在对局记录之前；点击任一姓名打开同一个安全区详情，显示双方实际存在的名次、段位/阵营称号、主宰称号、主宰与连接信息。关闭后立即恢复对战命中。
- 桌面信息区和移动详情共用`BattlePlayerIdentity`的数据语义；桌面保留常驻分行布局，移动常驻条及详情几何只在移动样式中定义。缺失名次或称号时整行省略，不生成“未上榜”“未定级”等伪状态。
- 士气摘要标题不再挂载旧阵营图节点；阵营效果入口和实际士气圆图保持原状态来源与交互。
- 主宰标记轨道固定在主宰网格第二行，双方使用同一槽尺寸、间距和contain图片规则；0—5个标记都以整条轨道居中，桌面的绝对坐标、裁切与缩放不再泄漏进移动画布。
- `/sandbox`作为既有显式逻辑横向路由，追加独立移动样式；它消费`visualViewport`与安全区令牌，在短横屏中提供移动头部/导航抽屉、滚动内容、双方换牌库、设置和粘性入口操作。没有设备身份、pointer或物理orientation推断。

...
- 聚焦四类既有真实测试与效果呈现：196/196通过；效果呈现专项47/47。覆盖四项目录和运行时解析、伏击成功/目标失效/检查点恢复、佣兵被无效不返还、傀儡成功与被无效、落穴只无效所选登场段。
- 完整Batch规则2915/2915通过，0失败、0跳过；公开响应声明、全卡池原子、私区事务、ST/S1/S2逐能力门禁全部通过。
- 无前端布局、卡图或部署改动；既有UI结果消费契约继续生效。

## 2026-09-12 单段主动实际结算首批

- 聚焦`EffectPresentationBranchSegmentTests|SingleActiveSelectionTests`：61/61通过，覆盖8项显式目录、唯一第1/1段、运行能力解析、白起真实成功、奥尔加真实无对象跳过、检查点恢复、重复提交、被无效和费用保留。
- Batch规则：2912/2912通过，0失败、0跳过；全卡池原子零旧case、主动/触发/响应/手牌公开声明、效果生成打出、私有区域事务、ST 76张/121项能力边界及S1/S2逐能力审计全部通过。
- 本批没有前端布局或资源修改；既有结构化结果消费路径由上批UI契约固定。未执行上传、部署或线上数据修改。

## 2026-09-06 BATCH-253 统一发布前验收
...
- 隔离验证：双方沙盒调度、合法及被拒绝 GM 命令仍进入本局权威事件流，但不创建 SQLite 正式对局记录，不进入个人记录、排行榜或排位结算。

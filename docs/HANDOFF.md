...
- **低卡顿性能治理第4批待 Main 对抗复核、未部署**：服务端新增覆盖全部 `/api` 的分级频率守门和统一异常边界。匿名读取120次/分钟、账号读取240次/分钟、普通写入60次/分钟、匿名认证与遥测各30次/分钟、高成本后台操作10次/分钟；分区以可信客户端或账号为准、内存最多8192项，429返回 `Retry-After`、剩余额度和关联 ID。好友玩家操作使用共享动作门闩，在途期间禁用并保留250毫秒短冷却；不在网络层合并真实写意图。服务端专项6/6、完整平台169/169、Batch平台160/160、规则4742/4742、动作门闩13/13、服务端架构10/10、请求可靠性30/30、UI341及正式/测试构建各322模块均通过，待提交级Release与 Main 对抗整合；不增加邮箱验证机制。

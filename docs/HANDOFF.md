...
- 当前进行中、未部署：16条已批准生产Bug中，共享触发调度已由`e492b3c`完成Release并同步main；第二个破坏/战斗续程批已完成开发Batch。效果击杀现在统一等待霍列姆赫布/海伦/阿喀琉斯等致命替代后恢复父堆叠，批量击杀以可恢复索引逐项处理；嬴政接受/拒绝/V2恢复、纷乱箭多目标和兰斯洛特进攻步行者罗洛同归均有具名回归。全池32个`KillTarget`调用/11个文件已扫描，精确4/4、相邻141/141、Focused与Batch规则4405/4405及静态审计通过。下一步提交、Release/main同步后继续资格与结构化规则批。全部16条完成后才交付Main一次部署，生产验证成功后解除维护。

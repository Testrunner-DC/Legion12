...
- 其他历史工作树的脏文件已逐项按内容比对：卡效相关源码与测试均已由后续主线实现覆盖；仅保留历史台账/交接快照，不将过时中间状态倒灌。整合候选已通过 UI 契约329项及字体、主题、音乐、连接恢复、回放保留、重入、维护围栏、资源投影、弹框/效果展示、个人页/后台、移动响应式、移动回放隔离和回放焦点等门禁；完整 Batch/Release 仍须在同步主线前执行。

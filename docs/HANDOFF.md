...
- **低卡顿性能治理第2批待 Main 整合**：页面轮询改为 WebSocket 资源版本/变化推送。在线状态直接推送查看者快照；好友改为定向失效通知加单个聚合概览，排位完整性、异画权益、运营策略按受影响范围通知；断线满60秒才启用统一低频兜底，隐藏页静默并保留最后确认数据。专项资源同步15/15、连接恢复25/25、真实双WebSocket好友通知1/1；对齐最新主线后的提交级 Release 规则4742/4742、平台154/154、UI341、正式/测试路径各316模块及发布包构建均通过。交由 L12-Main 对抗复核、统一推送，不部署且不包含邮箱验证。

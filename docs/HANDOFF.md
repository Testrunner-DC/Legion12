...
- **牌库编辑器参考流程批已在独立 worktree 完成，待 Main 对抗复核，未部署**：Gallery / Stats / Hand 使用同一当前构筑；移动端改为卡池、牌表、统计/起手单任务入口，不再强制横屏。主宰、主牌、士气、额外区、备选区可折叠并记忆；禁限与阵营问题就地呈现。私人备选区按卡号＋数量紧凑保存，不参与合法性或公开版本。定向存储5/5、完整平台168/168、完整规则4743/4743、编辑器合同18项、UI341、请求可靠性30/30、正式/测试构建各323模块及六档视口12张截图全部通过。范围见 `docs/DECK-EDITOR-REFERENCE-FLOW.md`。

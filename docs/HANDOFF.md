...
- 移动弹框内部自适应已完成本机收口，待提交级Release与同步，未部署。卡牌选择与墓地卡列现在保持完整卡图并显式显示完整卡名；单卡/少量卡在可用区居中，多卡自然横向滑动且不显示滚动条。主宰、阵营、卡牌效果的卡图与正文作为整体均匀分布，士气/符文选择在剩余内容区横纵居中，操作区稳定守底。四档12类共48张弹框截图、三组安全区18张和桌面3档隔离通过；UI契约331项、Vue类型检查及隔离生产构建通过。证据目录为 `artifacts/mobile-dialog-content-adaptive-final3` 与 `artifacts/mobile-dialog-content-adaptive-final-safe`。

...
- **低卡顿性能治理第1批已由 Main 接收整合**：以 TypeScript AST 锁定新增轮询、裸业务 `fetch`、超3路页面扇出、过期与陈旧豁免；门禁进入前端构建、分级验证和 GitHub main 工作流。原隔离提交 `e63d379` 已并入本批统一发布链，最终提交号待完整 Release 与推送后补记；未部署。

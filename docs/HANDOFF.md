...
## 当前状态（2026-09-24，覆盖下方过程记录）

- **牌库编辑器参考流程批已在独立 worktree 完成，待 Main 对抗复核，未部署**：Gallery / Stats / Hand 使用同一当前构筑；移动端改为卡池、牌表、统计/起手单任务入口，不再强制横屏。主宰、主牌、士气、额外区、备选区可折叠并记忆；禁限与阵营问题就地呈现。私人备选区按卡号＋数量紧凑保存，不参与合法性或公开版本。已合入 `origin/main@826f418b2a85ef8dc83db11a262a0d3c3222cac0` 的移动对战停靠修复；定向存储5/5、完整平台168/168、完整规则4743/4743、编辑器合同18项、UI341、请求可靠性30/30、合并后正式/测试构建各331模块及六档视口12张截图全部通过。范围见 `docs/DECK-EDITOR-REFERENCE-FLOW.md`。

- **公开牌库详情内容批已由 Main 最终验收并推送，未部署**：`origin/main` 已推进到 `a2c8c822ad7b45daa76d610a35b6c2a3889cf746`。既有详情页补齐指南、按敌方主宰的对局建议、全部长期版本与逐卡差异、权威版本对局空态和不落库的随机 6 张起手；内容正文/修订和全部构筑版本均不自动删除。验收含专项后端4/4、完整平台167/167、Batch规则4742/4742、请求可靠性30/30、UI341、正式/测试构建各323模块及六档视口12张截图；范围、迁移与回滚见 `docs/PUBLIC-DECK-DETAIL-CONTENT.md`。
...
- **低卡顿性能治理第3批已由 Main 完成对抗复核，未部署**：平台唯一 HTTP 出口新增4并发上限、覆盖排队/重试等待/响应解析的总截止时间、同账号安全读取在途合并及有限弱网/429重试。读取总预算10秒且最多2次尝试；普通写入20秒、媒体60秒，调用方只能收紧预算，写入无论配置均不自动重试或通用合并，避免吞掉玩家连续执行的真实操作。读取合并键仅保存不可逆摘要，不驻留令牌、查询参数或业务头原文；账号切换前后的旧请求均失败关闭，认证刷新禁止与通用读取形成嵌套重试。Main补强后的请求可靠性专项30/30、性能架构13/13、资源同步18/18、连接恢复25/25、UI341及完整Release通过：规则4742/4742、平台154/154、正式/测试构建各321模块和发布包成功。

...
- **牌库编辑器参考流程批已在独立 worktree 完成第一阶段，正按 Main 新批次继续收口，未部署**：Gallery / Stats / Hand 使用同一当前构筑；移动端改为卡池、牌表、统计/起手单任务入口，不再强制横屏。主宰、主牌、士气、额外区、备选区可折叠并记忆；禁限与阵营问题就地呈现。私人备选区按卡号＋数量紧凑保存，不参与合法性或公开版本。第一阶段范围见 `docs/DECK-EDITOR-REFERENCE-FLOW.md`；当前批继续处理删除确认、公开内容编辑闭环、详情同页锚点、横向卡、筛选抽屉、侧栏宽度与公开牌库二维码。

...
- 公开进攻我方军团目标复验族已完成、未部署：全池精确命中阿伊与稻姬本多小松。两条进攻入口现复用规则内核的当前我方军团出口，响应后重验同一实例仍为军团、仍在前排并满足兵力/有效阵营/来源条件；失效不补位、不退已付士气，统一记录失败而非取消。专项4/4，Focused与Batch规则4190/4190；干净Release规则4190/4190、平台144/144、UI325、连接23/23、防御23/23、资源投影9/9、牌库同步9/9、排序10/10、卡图324+42及生产构建全部通过、最终退出码0。本提交同步main；不改响应窗口，未部署。
- 上一批公开登场我方军团目标复验已由`428ad5a85fc389f09825c69c57ee2c2e948e24b6`同步main、未部署：墨子、拉美西斯、安卡神碑、卡诺匹斯罐一/四、安倍晴明、稻姬、井伊直虎共用单/多目标当前状态复验；完整规则与发布门禁已通过。

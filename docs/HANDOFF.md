...
- **牌库、移动对战、沙盒入口与测试服验收数据已形成统一测试服候选**：牌库11项闭环、移动士气/玩家信息/主宰圆标、沙盒前置页和桌面身份布局均已由 Main 接收；测试服只读验收数据严格受`/testrun`隔离与显式环境变量双重约束，不生成虚假赛果或统计。整合后牌库合同与二维码扫码、UI契约、移动127组交互、沙盒32组和身份28组均通过；二维码接入时同步把`sharp`升级到修复已知高危问题的`0.35.4`，`npm audit`为0。下一出口是完整 Release、统一推送和一次测试服部署，正式服不变。

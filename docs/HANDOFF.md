...
- 当前连续批次未部署：邪眼末日线索已扩扫为5条双方私密同时选择入口（另含迷雾绝境、神之天平、天启默示录、黑胡子蒂奇），共用“暂存各方选择、最后提交后统一结算”事务。邪眼双方视图/等待态、V2恢复、重复提交、对象失效、单方无目标及迷雾跨恢复已补；四族140/140、Focused/Batch规则4252/4252通过。下一步同批提交→Release/main同步，不因检查点停止。

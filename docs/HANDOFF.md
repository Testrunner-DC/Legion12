...
- **低卡顿性能治理第3批待 Main 对抗复核、未部署**：平台唯一 HTTP 出口新增4并发上限、排队截止、同账号安全读取在途合并、有限弱网/429重试和相同写动作单飞。读取10秒且最多2次尝试；普通写入20秒、媒体60秒，调用方只能收紧预算，写入无论配置均不自动重试。账号切换前后的旧请求均失败关闭，文件流和大型请求体不猜测合并；认证刷新禁止与通用读取形成嵌套重试。请求可靠性专项26/26、连接恢复25/25、UI契约341项、Vue类型检查和正式/测试构建各321模块均通过，待提交级 Release 与 L12-Main 对抗整合。

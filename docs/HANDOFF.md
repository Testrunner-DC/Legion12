...
- **架构P1已按用户确认启动，首批内核契约冻结已通过验收**：独立工作区基于`origin/main@6953e44`，已建立能力身份、内核端口、命令/Prompt/事件/快照、固定命令序列、检查点恢复、接收者隐私和初始读取顺序的可执行黄金夹具；新增依赖方向门禁，59个内核局部源码不得引入HTTP、房间、SQLite、录像写入器或文件系统。已吸收“L12-架构方案”评估：快照/序列化/哈希当前不全是纯读取，按三层事务保留房间提交顺序，17组有条件加入手牌公开出口列为待修而非规范，P3基于既有Journal V2演进，旧版本高成本回放允许明确失效，并在P1-B补性能基线。现有唯一`L12PlatformStore.AnnihilationCardId`历史常量依赖只记录不扩散，后续另批下沉。P1契约4/4、41个隔离部署行为场景、Focused/Batch以及Debug/Release规则4747/4747均通过；本批不改卡效、协议、存档或回放格式，不部署，统一推送后继续P1-B。

...
- 当前进行中、未部署：付费扩展射程全池仅攻城投石车与养由基2项。现有统一运行正确，新增严格生命周期档案绑定后排资格、每次支付、响应、结算来源复验、实际攻击候选/声明复验与回合到期；不把静态远程段混入，也不新增次数限制。档案/真实流程23/23、台账生成器13/13、相邻356/356、完整Batch规则4282/4282通过，两项已从待核对转为共用规则归属；待统一提交与Release。

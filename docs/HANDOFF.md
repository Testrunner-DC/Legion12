...
## 当前状态（2026-09-18，覆盖下方过程记录）

- 当前收口触发型“我方军团”目标：月读位移后进攻加成、迦具土固定目标增兵两条曾仅判断在战场；现改读当前我方军团，类型变化与相邻119/119、Batch全门禁通过；待干净Release、提交与main同步，不部署。
...
- 已落地的玩家侧适配：图鉴与公开牌库竖屏只保留搜索和“筛选”，完整条件进入底部面板；规则栏目由横向拖动改为两列换行；首页、资讯、牌库、个人页、规则中心在 ≤520px 使用紧凑的标题、说明和面板间距。
- 防遮挡：移动/矮横屏的 Bug 反馈由浮动按钮改为导航入口；牌库错误提示在移动端回到内容流中，不再盖住卡片操作。对局侧保留士气大面板入口、所选卡牌操作置于结束回合上方、对局记录可全屏查看的前序实现。
- 已完成本机视觉验收：320×640 竖屏首页/规则/个人/牌库/图鉴筛选，844×390 横屏真实方向；逐项确认导航、筛选展开/选择/关闭、反馈弹窗开关、无横向页面溢出。图鉴/公开牌库的实卡数据与实战操作仍需在带后端数据及真实对局状态的集成环境复验。
- 验证：`node scripts/check-mobile-responsive-layouts.mjs`、`npx vue-tsc --noEmit --skipLibCheck`、`git diff --check` 通过。本批未提交、未推送、未部署；不得把本机 `Failed to fetch`（未启动本地后端）描述为线上功能故障。

...
- 当前批次为严格手牌军团登场结算：完整扫描旧登场旁路后锁定刘备、西施、锡瓦的卡巴三条跨响应静默路径。共用`TrySummonFromHand`复验同一手牌实例、军团/能力条件、战场与位置；西施失败阻断随后抽牌，锡瓦失败不锁士气。能力档案与具名定向51/51，精确12文件Batch完整规则4078/4078、失败0、跳过0、退出0；待同批提交、干净Release和main同步，不部署。无规则待答。

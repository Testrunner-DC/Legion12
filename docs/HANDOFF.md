...
- 当前反击战术身份与公共禁用族已完成开发并本地提交、尚未同步或部署：18张统一为`tactic + isCounterTactic`，删除前后端运行时卡号表；同时离场去重、奈芙蒂斯候选预占/拒绝释放及宫廷魔术师普通响应/匿名盖伏/触发入队/声明前重验均已收口。初始分类1红、禁用4红；专项137/137、干净提交规则4087/4087、零失败/跳过，前端类型、本批UI分类契约、卡图架构通过。平台测试夹具已补齐新增身份字段，正重跑提交级Release。完整UI脚本另有3项既有移动端差异，不得误算本批失败或擅自覆盖；不部署。

...
- **玩家战报必要状态补回（未提交）**：保留批次 G 的强过滤/压缩，不恢复失败、取消、堆叠、响应或服务端原文；补回资源/手牌、试炼进度、公开卡牌区域、效果目标状态、天灾值和额外回合。彼界阵营结果改为 `runes` 权威事件，通常试炼行动与进度合并显示；战场军团移动只显示“已移动”，不显示格数或坐标。日志投影 63 行、UI 340、Vue 类型检查、彼界定向 2/2及完整规则 4742/4742 均通过；仅有 `NU1900` 环境警告。当前改动留在 `D:/GPT/Legion12/app`，按用户指示交由 **L12-main 对话**统一提交，不在本任务提交或推送。

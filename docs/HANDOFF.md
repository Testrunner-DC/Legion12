...
- **公开牌库详情内容批已在独立工作树完成，待 Main 对抗复核，未部署**：既有详情页补齐指南、按敌方主宰的对局建议、全部长期版本与逐卡差异、权威版本对局空态和不落库的随机 6 张起手。SQLite v5 对作者内容做限长纯文本校验、正文哈希去重与追加修订引用；内容正文/修订和全部构筑版本均不自动删除。没有可靠的“公开版本—对局”绑定时明确空置，不拿作者总战绩替代。专项后端4/4、存储相邻15/15、完整平台167/167、Batch规则4742/4742、Batch平台158/158、UI341、生产/测试构建及六档视口12张截图通过；范围、迁移与回滚见 `docs/PUBLIC-DECK-DETAIL-CONTENT.md`。
...
- **低卡顿性能治理第2批已由 Main 接收、未部署**：页面轮询改为 WebSocket 资源版本/变化推送。在线状态直接推送查看者快照；好友改为定向失效通知加单个聚合概览，排位完整性、异画权益、运营策略按受影响范围通知；断线满60秒才启用统一低频兜底，隐藏页静默并保留最后确认数据。Main 对抗复核发现并修正“切换账号后旧请求的 finally 可能清空新账号进行中请求”竞态，重叠账号切换回归纳入资源同步门禁（18/18）；待组合批次最终 Release 和统一推送，不部署且不包含邮箱验证。

...
- **2026-09-26 当前权威出口（覆盖下方历史“待推送／待部署”文字）**：`origin/main` 与干净候选均为 `b7d9f6306b9b6b518ce948325e7ed7ee01b2c485`。同提交 Release 规则4766/4766、平台205/205、UI契约348项、正式/测试双构建通过；已部署独立测试服，公网健康、HTTP与WebSocket精确匹配该提交。正式服仍为 `fdb73f84d058a415842d071a7a4cb92776352556`。当前只剩状态文档提交后的再次 Batch/Release、正式服部署、25+3+3+1 Bug线上闭环及旧工作树差异归档/清理；下方历史条目只作证据，不再表示仍待合入。

- **2026-09-26 最新对战收口**：非战斗离场不再因同批视觉事件被吞掉，主宰效果卡图与区域移动按权威序号统一播放；勇士比约恩墓地加权多选不再让“不发动”占用序号，支付后按声明位置登场，位置后被占用时留墓且不退费。移动选择弹框和计时模块已纳入对抗验收：顺序协调7/7、离场投影28/28、动画浏览器51项/10帧、计时419项/28图、服务端完整4766/4766。

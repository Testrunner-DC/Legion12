...
- 当前开发批次未部署：移动对战标准弹框已统一为安全逻辑画布内的等比阅读框。宽、高占用各以安全画布 `75%` 为上限，并保持约 `16:9`，超宽手机与4:3平板只做等比 contain，不再随设备比例压扁或拉长；长内容在框内滚动。Prompt/等待、墓地、主宰、阵营与卡牌效果、记录、士气和设置共用；卡牌详情抽屉、底部支付条与战场直选条排除。四档48张弹框截图、三组安全区18张、桌面3档隔离、UI331、类型与生产构建均通过；证据位于 `artifacts/mobile-dialog-75-ratio-all` 和 `artifacts/mobile-dialog-75-ratio-quick`。待统一提交推送，不部署。

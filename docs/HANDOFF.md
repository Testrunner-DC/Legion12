...
## 2026-09-17 支援阵亡与进攻方击杀事实分离（功能已提交，待Batch验证）

- 支援军团显式按`Defeat`离场：自身【阵亡时】继续进入通用候选；不会写入交战`Defeated*InstanceId`或产生`kill-source`，所以进攻军团不触发【击杀时】。
- 新增武则天支援、源义经进攻的V2恢复端到端回归：支援者入墓后抽牌并治疗，攻击方没有任何击杀来源。功能提交`e01cd28`；战斗时间线、类型化击杀、协防/近藤相邻126/126及Focused规则3302/3302通过；尚待Batch和Release，未部署。

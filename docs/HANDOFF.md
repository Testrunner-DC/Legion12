...
- **低卡顿性能治理第5批完成Main对抗补强、待统一推送，未部署**：API增加固定容量的最近60秒运行时性能窗口，不可变分桶代际和桶内原子快照避免轮换污染与撕裂读取；管理员页展示有效读取/写入样本、独立诊断请求、在途/峰值、平均/P95延迟档、慢请求、429、真实5xx、计划性503和499。状态页刷新不进入验收分母，计划性503与客户端断开不伪装成服务器故障；不按路径或玩家建高基数分区，不逐请求落盘，也不新增轮询。上线门禁要求至少20个有效样本（读取10、写入2）、75秒内快照且运行版本精确等于目标40位提交，慢请求不高于5%、真实5xx不高于1%、429不高于20%，并生成不可覆盖验收回执。本批不部署、不涉及邮箱验证。

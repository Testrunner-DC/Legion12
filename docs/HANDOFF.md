...
- “无法因效果转为活跃”族已完成开发目录验证，待同批提交级Release：公共资格覆盖可选择对象与`effect-ready`响应后结算；芬尼亚、井伊直虎、芬恩不再直接写活跃状态。冒号前Cost与后段效果保持分离，费用合法即可发动，禁止状态只让转活跃段跳过/失败且不退款。新增旁路守卫；同族宽测499/499、冒号边界17/17、完整规则4154/4154通过。未部署。
